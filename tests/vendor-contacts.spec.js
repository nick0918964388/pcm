...
test.describe('廠商聯絡人顯示測試', () => {
    ...
    if ((await vendorLink.count()) > 0) {
      ...
      if (vendorCount > 0) {
        ...
        if ((await viewContactsButton.count()) > 0) {
          ...
          // 2. 檢查廠商資訊卡片
          const vendorInfoCard = await page
          ...
          const statsCards = await page.locator(
            '[class*="grid"] >> text=/總聯絡人|活躍聯絡人|主要聯絡人/'
          );
          ...
          const contactCards = await page.locator(
            '[class*="ContactCard"], [class*="contact-card"]'
          );
          ...
          if (contactCount === 0) {
            // 檢查空狀態訊息
            const emptyMessage =
              await page.getByText(/尚無聯絡人資料|找不到符合條件的聯絡人/);
            ...
            const contactName = await firstContact
              .locator('[class*="font-bold"], [class*="font-semibold"], h3, h4')
              .first();
      ...
      if ((await viewContactsButton.count()) > 0) {
        ...
        if ((await searchInput.count()) > 0) {
          ...
          const filteredResults = await page.locator(
            '[class*="ContactCard"], [class*="contact-card"], text=/找不到符合條件/'
          );
        ...
        // 測試檢視模式切換
        const gridButton = await page
          .locator('[aria-label*="grid"], button:has(svg[class*="Grid"])')
          .first();
        const listButton = await page
          .locator('[aria-label*="list"], button:has(svg[class*="List"])')
          .first();
  ...
  test('測試聯絡人互動功能', async ({ page }) => {
    ...
    if ((await vendorLink.count()) > 0) {
      ...
      if ((await viewContactsButton.count()) > 0) {
        ...
        // 檢查新增聯絡人按鈕
        const addContactButton = await page.getByRole('button', {
          name: /新增聯絡人|add contact/i,
        });
        if ((await addContactButton.count()) > 0) {
          ...
          if ((await dialog.count()) > 0) {
            ...
            const closeButton = await page
              .locator(
                '[aria-label*="close"], button:has-text("取消"), button:has-text("關閉")'
              )
              .first();
        ...
        if ((await contactCards.count()) > 0) {
          ...
          // 檢查編輯按鈕
          const editButton = await firstCard
            .locator(
              'button[aria-label*="edit"], button:has(svg[class*="Edit"]), button:has(svg[class*="Pencil"])'
            )
            .first();
          ...
          // 檢查電話按鈕
          const phoneButton = await firstCard
            .locator(
              'button[aria-label*="phone"], button:has(svg[class*="Phone"])'
            )
            .first();
          ...
          // 檢查郵件按鈕
          const emailButton = await firstCard
            .locator(
              'button[aria-label*="email"], button:has(svg[class*="Mail"])'
            )
            .first();
  ...
  test('測試響應式設計', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      console.log(
        `測試 ${viewport.name} 視窗 (${viewport.width}x${viewport.height})`
      );
      ...
      if ((await vendorLink.count()) > 0) {
        ...
        if ((await viewContactsButton.count()) > 0) {
          ...
          const mainContent = await page
            .locator('[class*="container"], main, [role="main"]')
            .first();
