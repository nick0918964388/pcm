...
test.describe('廠商聯絡人資料庫驗證測試', () => {
  ...
  test('驗證聯絡人資料與資料庫一致', async ({ page }) => {
    ...
    if (vendorLink) {
      ...
      if (await vendorCard.count() > 0) {
        ...
        // 尋找查看聯絡人按鈕
        const viewContactsButton = await page.locator(`text=${firstVendor.name} >> .. >> button:has-text(/聯絡人|contact/i)`).first();
        if (await viewContactsButton.count() > 0) {
          ...
          // 獲取該廠商的聯絡人資料
          const vendorContacts = dbContacts.filter(c => c.vendor_id === firstVendor.id);
          console.log(`該廠商有 ${vendorContacts.length} 個聯絡人`);
          
          // 驗證頁面上顯示的聯絡人數量
          const contactCards = await page.locator('[class*="card"]:has-text(/電話|email|phone/i)');
          const displayedContactsCount = await contactCards.count();
          console.log(`頁面顯示 ${displayedContactsCount} 個聯絡人卡片`);
          
          // 驗證每個聯絡人的資料
          for (const contact of vendorContacts.slice(0, 3)) { // 只驗證前3個避免太長
            console.log(`\n驗證聯絡人: ${contact.name}`);
            
            // 檢查姓名是否顯示
            const nameElement = await page.locator(`text=${contact.name}`).first();
            ...
            // 檢查職稱是否顯示
            if (contact.title) {
              const titleElement = await page.locator(`text=${contact.title}`).first();
            ...
            if (contact.department) {
              const deptElement = await page.locator(`text=${contact.department}`).first();
            ...
            if (contact.phone) {
              ...
              const phoneElement = await page.locator(`text=/${phoneText}|${contact.phone}/`).first();
            ...
            // 檢查 Email 是否顯示
            if (contact.email) {
              const emailElement = await page.locator(`text=${contact.email}`).first();
          ...
          // 驗證統計資料
          const statsSection = await page.locator('text=/總聯絡人|活躍聯絡人/').first();
          if (await statsSection.count() > 0) {
            console.log('\n統計資料區域已顯示');
            
            // 檢查總聯絡人數
            const totalContactsElement = await page.locator('text=/總聯絡人/ >> .. >> text=/\\d+/').first();
  ...
  test('測試聯絡人搜尋功能與資料庫資料匹配', async ({ page }) => {
    ...
    // 導航到廠商管理頁面
    const vendorLink = await page.locator('text=/廠商管理|供應商|Vendor/i').first();
    if (await vendorLink.count() > 0) {
      ...
      if (await viewContactsButton.count() > 0) {
        ...
        if (await searchInput.count() > 0) {
          ...
          // 驗證搜尋結果
          const searchResult = await page.locator(`text=${testContact.name}`).first();
          if (await searchResult.count() > 0) {
            ...
            if (testContact.title) {
              const titleInResult = await page.locator(`text=${testContact.title}`).first();
            ...
            if (testContact.email) {
              const emailInResult = await page.locator(`text=${testContact.email}`).first();
  ...
  test('驗證聯絡人狀態與資料庫一致', async ({ page }) => {
    ...
    if (await vendorLink.count() > 0) {
      ...
      if (await viewContactsButton.count() > 0) {
        ...
        // 檢查統計卡片中的活躍聯絡人數
        const activeStatsCard = await page.locator('text=/活躍聯絡人/ >> .. >> text=/\\d+/').first();
        ...
        const primaryStatsCard = await page.locator('text=/主要聯絡人/ >> .. >> text=/\\d+/').first();
