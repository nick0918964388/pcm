...
export function usePhotoPerformance(
  options: UsePhotoPerformanceOptions
): UsePhotoPerformanceReturn {
  const {
    photos,
    containerHeight,
    itemHeight,
  ...
  const checkCache = useCallback(
    async (photoId: string): Promise<Blob | null> => {
      ...
      try {
        const transaction = cacheRef.current.transaction(
          ['images'],
          'readonly'
        );
  ...
  const cacheImage = useCallback(
    async (photoId: string, imageData: Blob): Promise<void> => {
    ...
    (photoId: string) => {
      if (enablePerformanceMonitoring && loadTimersRef.current[photoId]) {
        ...
        setPerformanceMetrics(prev => {
          ...
          const totalLoadTime = Object.values(newLoadTimes).reduce(
            (sum, time) => sum + time,
            0
          );
          const averageLoadTime =
            totalLoadTime / Object.keys(newLoadTimes).length;
