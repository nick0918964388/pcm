...
export const useWBSTreeQuery = (
  ...
  return useQuery({
    queryKey: ['wbsTree', projectId, { filters, includeCompleted, maxDepth, sortBy, sortOrder }],
    queryFn: async () => {
      const queryOptions = {
        ...(filters && { filters }),
        includeCompleted,
...
export const useWBSOperations = (projectId: string) => {
  ...
  const updateNodeMutation = useMutation({
    mutationFn: async ({ nodeId, nodeData }: { nodeId: string, nodeData: Partial<WBSItem> }) => {
      const result = await apiService.updateWBSNode(projectId, nodeId, nodeData)
      return result.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['wbsTree', projectId] })
  ...
  const updateNode = useCallback(async (
    ...
    try {
      const result = await updateNodeMutation.mutateAsync({ nodeId, nodeData })
      options.onSuccess?.(result)
      return result
    } catch (error) {
      options.onError?.(error as Error)
  ...
  const moveNode = useCallback(async (
    ...
    try {
      const result = await moveNodeMutation.mutateAsync({ nodeId, newParentId, newIndex })
      options.onSuccess?.(result)
      return result
    } catch (error) {
      options.onError?.(error as Error)
...
export const getWBSTreeQueryKey = (
  ...
  const { filters, includeCompleted = true, maxDepth, sortBy, sortOrder } = options
  return ['wbsTree', projectId, { filters, includeCompleted, maxDepth, sortBy, sortOrder }]
}
 ...
 * @param projectId 專案 ID
 */
export const invalidateWBSQueries = (
  queryClient: any,
  projectId?: string
) => {
  if (projectId) {
