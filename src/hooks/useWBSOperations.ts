...
export const useWBSDragDrop = (projectId: string) => {
  ...
  const startDrag = useCallback(
    (nodeId: string, operationType: DragOperationType = 'move') => {
      setDragState({
        isDragging: true,
  ...
  const dragOver = useCallback(
    (targetNodeId: string, dropPosition: 'before' | 'after' | 'inside') => {
      setDragState(prev => ({
        ...prev,
        hoveredNodeId: targetNodeId,
        dropPosition,
...
export const useWBSBatchOperations = (projectId: string) => {
  ...
  const batchUpdate = useCallback(
    async (nodeIds: string[], updates: Partial<WBSItem>) => {
  ...
  const batchChangeStatus = useCallback(
    async (nodeIds: string[], status: string) => {
      const request: BatchUpdateWBSRequest = {
        operation: 'update',
        nodeIds,
        data: { status },
...
export const useWBSValidation = (tree: WBSItem[]) => {
  const [validationResult, setValidationResult] =
    useState<WBSValidationResult | null>(null);
...
export const useWBSImportExport = (projectId: string) => {
  ...
  // 導入 WBS
  const importWBS = useCallback(
    async (file: File, options: ImportExportOptions = { format: 'excel' }) => {
      setIsImporting(true);
      setImportProgress(0);
