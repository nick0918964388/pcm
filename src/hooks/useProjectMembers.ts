...
export const useProjectMembersQuery = (
  ...
  return useQuery({
    queryKey: ['projectMembers', projectId, { page, pageSize, filters, sortBy, sortOrder }],
    queryFn: async () => {
      const queryOptions = {
        page,
        pageSize,
...
export const getProjectMembersQueryKey = (
  ...
  return ['projectMembers', projectId, { page, pageSize, filters, sortBy, sortOrder }]
}
...
export function useUpdateProjectMember(projectId: string) {
  ...
  return useMutation({
    mutationFn: async ({ memberId, data }: { memberId: string, data: UpdateProjectMemberRequest }) => {
      const apiService = new ProjectStaffWBSApiService()
