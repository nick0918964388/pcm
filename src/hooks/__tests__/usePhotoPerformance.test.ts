...
describe('usePhotoPerformance', () => {
  ...
  describe('懶載入機制', () => {
    it('應該初始化時不載入任何圖片', () => {
      const { result } = renderHook(() => usePhotoPerformance({
        photos: mockPhotos,
        containerHeight: 600,
        itemHeight: 200,
        threshold: 0.1
  ...
  describe('效能監控', () => {
    it('應該追蹤載入時間', () => {
      ...
      expect(result.current.performanceMetrics.loadTimes['photo-0']).toBeGreaterThan(0)
    })
    ...
    it('應該計算平均載入時間', () => {
      ...
      expect(result.current.performanceMetrics.averageLoadTime).toBeGreaterThan(0)
    })
  })
