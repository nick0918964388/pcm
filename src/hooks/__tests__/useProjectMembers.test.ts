...
describe('useProjectMembersQuery', () => {
  ...
  it('應該成功載入專案人員資料', async () => {
    ...
    // Act
    const { result } = renderHook(() => useProjectMembersQuery('proj-001'), {
      wrapper,
    });
    ...
    expect(mockApiInstance.queryProjectMembers).toHaveBeenCalledWith(
      'proj-001',
      {
        page: 1,
        pageSize: 10,
  ...
  it('應該正確處理錯誤狀態', async () => {
    ...
    mockApiInstance.queryProjectMembers.mockRejectedValue(
...
describe('useProjectMemberSearch', () => {
  ...
  it('應該提供搜索建議', async () => {
    ...
    // Act
    const { result } = renderHook(() => useProjectMemberSearch('proj-001'), {
      wrapper,
    });
    ...
    expect(mockApiInstance.searchProjectMembers).toHaveBeenCalledWith(
      'proj-001',
      '張'
    );
  });
...
describe('useProjectMemberStats', () => {
  ...
  it('應該載入專案人員統計資料', async () => {
    ...
    // Act
    const { result } = renderHook(() => useProjectMemberStats('proj-001'), {
      wrapper,
    });
    ...
    expect(mockApiInstance.getProjectMemberStats).toHaveBeenCalledWith(
      'proj-001'
    );
  });
});
