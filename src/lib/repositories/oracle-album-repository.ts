...
export class OracleAlbumRepository extends OracleBaseRepository<Album> {
  ...
  protected mapToOracle(entity: Partial<Album>): Record<string, any> {
    ...
    if (entity.coverPhotoId !== undefined)
      row.cover_photo_id = entity.coverPhotoId;
  ...
  async getAlbumById(
    ...
    // 檢查權限
    const hasAccess = await this.validateAlbumAccess(
      albumId,
      userId,
      'read',
      options
   ...
   * Requirements: 1.6
   */
  async getAlbumDeletionInfo(
    albumId: string,
    options: OracleQueryOptions = {}
  ): Promise<AlbumDeletionInfo | null> {
    const sql = `
      SELECT
        pa.*
      FROM photo_albums pa
   ...
   * Requirements: 1.6
   */
  async deleteAlbumWithPermissionCheck(
    albumId: string,
    userId: string,
    options: OracleQueryOptions = {}
  ): Promise<boolean> {
    // 檢查使用者是否有管理權限
    const hasPermission = await this.validateAlbumAccess(
      albumId,
      userId,
      'delete',
      options
  ...
  async createAuditLog(
    auditData: Omit<AlbumAuditLog, 'id' | 'createdAt'>,
    options: OracleQueryOptions = {}
  ): Promise<void> {
    const sql = `
   ...
   * Requirements: 1.6
   */
  async getAlbumAuditLogs(
    albumId: string,
    options: OracleQueryOptions = {}
  ): Promise<AlbumAuditLog[]> {
    const sql = `
      SELECT
        id, album_id, user_id, action, reason, created_at, metadata
  ...
  async safeDeleteAlbum(
    ...
    if (force && deletionInfo.photoCount > 0) {
      result = await this.deleteAlbumWithForce(
        albumId,
        userId,
        true,
        queryOptions
  ...
  // ============================================
  // Metadata Export Support (Task 8.3)
  // ============================================
