...
export class OracleProjectRepository extends OracleBaseRepository<Project> {
  ...
  protected mapFromOracle(row: any): Project {
    return {
      ...
      // Oracle JSON處理
      client_info: row.CLIENT_INFO ? this.parseOracleJson(row.CLIENT_INFO) : null,
   ...
   */
  async findByProjectCode(code: string, options: OracleQueryOptions = {}): Promise<Project | null> {
    const sql = `
  ...
  async updateMetadata(
    ...
    return await this.updateJson(id, `metadata.${metadataPath}`, value, options)
  }

  /**
   * 查找逾期項目（使用Oracle日期函數）
   */
  async findOverdueProjects(options: OracleQueryOptions = {}): Promise<Project[]> {
    const sql = `
      SELECT * FROM ${this.getTableName()}
   ...
   * 查找即將到期的項目
   */
  async findUpcomingDeadlines(days: number = 7, options: OracleQueryOptions = {}): Promise<Project[]> {
    const sql = `
      SELECT * FROM ${this.getTableName()}
      WHERE ${this.config.softDeleteColumn} IS NULL
  ...
  async updateProgress(
    ...
    const queryExecutor = this.getQueryExecutor()
    const result = await queryExecutor.execute(sql, { progress, projectId }, options)
  ...
  async updateStatus(
    ...
    const queryExecutor = this.getQueryExecutor()
    const result = await queryExecutor.execute(sql, { status, projectId }, options)
    return (result.rows?.length || 0) > 0
  }
   ...
   */
  async upsertProject(project: Partial<Project>, options: OracleQueryOptions = {}): Promise<Project> {
    if (!project.id) {
