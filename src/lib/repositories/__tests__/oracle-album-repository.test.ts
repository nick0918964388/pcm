...
describe('OracleAlbumRepository', () => {
  ...
  describe('建立相簿 (Create Album)', () => {
    ...
    it('應該在專案代碼和相簿名稱重複時拋出錯誤', async () => {
      ...
      // Act & Assert
      await expect(repository.createAlbum(albumData))
        .rejects.toThrow('相簿名稱已存在於此專案中')
    })
  })
  ...
  describe('驗證相簿存取權限 (Validate Album Access)', () => {
    it('應該對有權限的使用者返回 true', async () => {
      ...
      // Act
      const result = await repository.validateAlbumAccess(albumId, userId, action)
  ...
  describe('安全刪除相簿機制 (Safe Album Deletion)', () => {
    ...
    it('應該拒絕刪除包含照片的相簿', async () => {
      ...
      mockQueryExecutor.execute.mockResolvedValue({
        rows: [{ photo_count: 5, project_id: 'proj001', name: '包含照片的相簿' }]
      })

      // Act & Assert
      await expect(repository.deleteAlbum(albumId, userId))
        .rejects.toThrow('無法刪除包含照片的相簿，請先刪除相簿內的所有照片')
    })
    ...
    it('應該在強制刪除時清理所有關聯照片', async () => {
      ...
      mockQueryExecutor.execute
        .mockResolvedValueOnce({
          rows: [{ photo_count: 3, project_id: 'proj001', name: '要強制刪除的相簿' }]
        })
        // 模擬軟刪除關聯照片
      ...
      // Act
      const result = await repository.deleteAlbumWithForce(albumId, userId, forceDelete)
      ...
      // 驗證刪除照片的 SQL（第二次調用）
      expect(mockQueryExecutor.execute).toHaveBeenNthCalledWith(2,
      ...
      // 驗證軟刪除相簿的 SQL（第三次調用）
      expect(mockQueryExecutor.execute).toHaveBeenNthCalledWith(3,
        expect.stringContaining('UPDATE photo_albums SET deleted_at = SYSDATE'),
        expect.objectContaining({ id: albumId }),
        {}
      )
    ...
    it('應該記錄刪除操作的稽核日誌', async () => {
      ...
      // Act
      const result = await repository.deleteAlbumWithAudit(albumId, userId, '刪除測試相簿')
    ...
    it('應該檢查使用者是否有刪除權限', async () => {
      ...
      // Act & Assert
      await expect(repository.deleteAlbumWithPermissionCheck(albumId, userId))
