...
describe('OraclePhotoRepository', () => {
  ...
  describe('照片 Metadata 管理 (Photo Metadata)', () => {
    it('應該更新照片的 metadata', async () => {
      ...
      // Act
      const result = await repository.updatePhotoMetadata(
        photoId,
        metadataUpdates
      );
  ...
  describe('照片管理操作 (Photo Management Operations)', () => {
    it('應該重新命名照片檔案', async () => {
      ...
      // 驗證更新 SQL (更新後的實作使用不同的參數名稱)
      expect(mockQueryExecutor.execute).toHaveBeenCalledWith(
        expect.stringContaining('SET file_name = :newFileName'),
        expect.objectContaining({
          newFileName,
          photoId,
        }),
    ...
    it('應該更新照片描述', async () => {
      ...
      // Act
      const result = await repository.updatePhotoDescription(
        photoId,
        newDescription
      );
