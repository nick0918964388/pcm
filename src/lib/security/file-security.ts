...
export class FileSecurityService {
  ...
  private readonly UPLOAD_RATE_LIMIT = new Map<
    string,
  ...
  async validateFileSecurely(
    ...
    if (file.size === 0) {
      ...
      errors.push(
        `檔案大小超過限制 (${this.formatFileSize(fileQuota.maxFileSize)})`
      );
    }

    // 5. 檢查上傳頻率限制
    const rateLimitCheck = this.checkUploadRateLimit(
      userId,
      fileQuota.maxFilesPerHour
    );
