...
export class RateLimitService {
  ...
  checkRateLimit(
    ...
    const now = Date.now();
    const record =
      store.get(key) || this.createNewRecord(now + config.windowMs);
   ...
   * 手動封鎖用戶
   */
  blockUser(type: string, identifier: string, durationMs: number): void {
  ...
  private getConfig(
    ...
    const baseConfig = RATE_LIMIT_CONFIGS[
      type as keyof typeof RATE_LIMIT_CONFIGS
    ] || {
      maxRequests: 100,
      windowMs: 60 * 60 * 1000,
    };
