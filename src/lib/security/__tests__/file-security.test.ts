...
describe('FileSecurityService', () => {
  ...
  describe('generateSecureFilePath', () => {
    test('應該生成安全的檔案路徑', () => {
      ...
      expect(path).toMatch(
        /^uploads\/[a-f0-9]{8}\/[a-f0-9]{8}\/\d{4}\/\d{2}\/\d{2}\/[a-f0-9-]{36}\.jpg$/
      );
    ...
    test('應該為相同輸入生成不同路徑', () => {
      const path1 = fileSecurityService.generateSecureFilePath(
        'proj',
        'user',
        'test.jpg'
      );
      const path2 = fileSecurityService.generateSecureFilePath(
        'proj',
        'user',
        'test.jpg'
      );
  ...
  describe('checkUserQuota', () => {
    test('應該拒絕超過大小限制的檔案', async () => {
      ...
      const result = await fileSecurityService.checkUserQuota(
        'user1',
        2048,
        quota
      );
