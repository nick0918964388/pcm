...
describe('RateLimitService', () => {
  ...
  describe('checkRateLimit', () => {
    ...
    test('應該拒絕超過限制的請求', () => {
      ...
      // 下一個請求應該被拒絕
      const result = rateLimitService.checkRateLimit('test', 'user1', {
        maxRequests,
        windowMs: 60000,
      });
    ...
    test('應該為不同用戶獨立計算', () => {
      ...
      // 用戶2應該仍然可以請求
      const result = rateLimitService.checkRateLimit('test', 'user2', {
        maxRequests,
        windowMs: 60000,
      });
    ...
    test('應該在時間窗口重置後允許新請求', () => {
      ...
      // 應該被拒絕
      let result = rateLimitService.checkRateLimit('test', 'user1', {
        maxRequests,
        windowMs,
      });
      expect(result.allowed).toBe(false);
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          result = rateLimitService.checkRateLimit('test', 'user1', {
            maxRequests,
            windowMs,
          });
          expect(result.allowed).toBe(true);
  ...
  describe('checkIPRateLimit', () => {
    ...
    test('應該接受自訂IP限制參數', () => {
      ...
      const result = rateLimitService.checkIPRateLimit(
        '192.168.1.2',
        maxRequests,
        windowMs
      );
  ...
  describe('checkUserRateLimit', () => {
    ...
    test('應該使用不同的限制配置', () => {
      ...
      const uploadResult = rateLimitService.checkUserRateLimit(
        'user1',
        'upload'
      );
