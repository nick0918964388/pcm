...
export class ContactAPI {
   ...
   * 取得聯絡人列表
   */
  static async getContacts(options: ContactQueryOptions = {}): Promise<ContactApiResponse<VendorContact[]>> {
    ...
    if (options.status?.length) searchParams.set('status', options.status.join(','))
    if (typeof options.isPrimary === 'boolean') searchParams.set('is_primary', options.isPrimary.toString())
    if (typeof options.isActive === 'boolean') searchParams.set('is_active', options.isActive.toString())
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        return {
          ...
          pagination: {
            ...
            totalPages: Math.ceil((result.total || contacts.length) / (options.pageSize || 20)),
            hasNext: false,
            hasPrev: false
          },
          stats: result.stats
   ...
   * 建立新聯絡人
   */
  static async createContact(contactData: Partial<VendorContact>): Promise<VendorContact> {
    try {
      // 轉換前端格式為 API 格式
      const apiData = {
   ...
   * 更新聯絡人資料
   */
  static async updateContact(contactId: string, contactData: Partial<VendorContact>): Promise<VendorContact> {
    try {
      // 轉換前端格式為 API 格式
      const apiData = {
        name: contactData.name,
  ...
  static async exportContacts(
    ...
    if (typeof filters.isPrimary === 'boolean') searchParams.set('is_primary', filters.isPrimary.toString())
