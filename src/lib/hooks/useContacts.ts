...
export function useContactMutation() {
  ...
  const createContact = useCallback(
    async (contactData: Partial<VendorContact>) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : '建立聯絡人失敗';
  ...
  const updateContact = useCallback(
    async (contactId: string, contactData: Partial<VendorContact>) => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : '更新聯絡人失敗';
        setError(errorMessage);
        throw err;
      } finally {
        setLoading(false);
  ...
  const deleteContact = useCallback(async (contactId: string) => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : '刪除聯絡人失敗';
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
