...
export class LocalFileStorageService {
  private readonly baseDirectory = PATH_SECURITY.BASE_DIRECTORY
  ...
  async uploadFile(
    ...
    try {
      ...
      const fileName = options?.preserveOriginalName ? file.name : this.sanitizeFileName(file.name)
   ...
   * 檢查檔案是否存在
   */
  async checkFileExists(filePath: string): Promise<Result<boolean, StorageError>> {
    try {
   ...
   * 確保目錄存在
   */
  async ensureDirectoryExists(directoryPath: string): Promise<Result<void, StorageError>> {
    try {
      // 檢查目錄是否已存在
      try {
   ...
   * 獲取目錄檔案列表
   */
  async getDirectoryListing(directoryPath: string): Promise<Result<FileInfo[], StorageError>> {
    try {
   ...
   * 驗證路徑安全性
   */
  private validatePathSecurity(projectCode: string, albumName: string): {
    isValid: boolean
    errors: string[]
  } {
    const errors: string[] = []
  ...
  private validateFileName(fileName: string): {
    ...
    if (fileName.length > PATH_SECURITY.MAX_FILENAME_LENGTH) {
      errors.push(`檔案名稱過長 (最大${PATH_SECURITY.MAX_FILENAME_LENGTH}字符)`)
    }
   ...
   * 檢查並獲取現有目錄資訊
   */
  private async getExistingDirectory(albumPath: string): Promise<DirectoryInfo | null> {
    try {
      await fs.access(albumPath)
      const stats = await fs.stat(albumPath)

      if (stats.isDirectory()) {
        const pathParts = albumPath.replace(this.baseDirectory + path.sep, '').split(path.sep)
        return {
          path: albumPath,
   ...
   * 建立檔案資訊物件
   */
  private async createFileInfo(filePath: string, originalFile: File, fileName: string): Promise<FileInfo> {
    const stats = await fs.stat(filePath)
