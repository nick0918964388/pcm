...
describe('EnhancedFileSecurityService', () => {
  ...
  beforeEach(() => {
    ...
    service = new EnhancedFileSecurityService(
      mockLocalStorage,
      mockFileSecurity
    );
    vi.clearAllMocks();
  ...
  describe('detectFileConflict', () => {
    it('應該檢測檔案名稱衝突並提供解決方案', async () => {
      ...
      mockLocalStorage.resolveFileConflict.mockImplementation(
        (path, strategy) => {
          if (strategy === 'rename') {
  ...
  describe('integratedValidation', () => {
    it('應該執行完整的整合驗證流程', async () => {
      ...
      // Act
      const result = await service.integratedValidation(
        file,
        projectId,
        albumName,
        userId
    ...
    it('應該阻止超過配額的檔案上傳', async () => {
      // Arrange
      const file = new File(['large file content'], 'large.jpg', {
        type: 'image/jpeg',
      });
      const projectId = 'TEST001';
      const albumName = '測試相簿';
      ...
      mockFileSecurity.validateFileSecurely.mockResolvedValue({
        ...
        errors: [],
        warnings: [],
        sanitizedFilename: 'large.jpg',
      });

      mockLocalStorage.getStorageQuota.mockResolvedValue({
        success: true,
        data: {
          totalUsed: 1020 * 1024 * 1024,
          totalLimit: 1024 * 1024 * 1024,
