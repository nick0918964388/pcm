...
import { DirectoryInfo, FileInfo, StorageError, ConflictResolution } from '../types'
...
describe('LocalFileStorageService', () => {
  ...
  describe('createAlbumDirectory', () => {
    ...
    it('應該回傳錯誤當專案代碼包含不安全字符', async () => {
      ...
      // Act
      const result = await service.createAlbumDirectory(dangerousProjectCode, albumName)
    ...
    it('應該回傳錯誤當相簿名稱包含路徑遍歷字符', async () => {
      ...
      // Act
      const result = await service.createAlbumDirectory(projectCode, dangerousAlbumName)
  ...
  describe('uploadFile', () => {
    it('應該成功上傳檔案到相簿目錄', async () => {
      ...
      const expectedPath = path.join(testBaseDir, projectCode, albumName, 'test.jpg')
    ...
    it('應該檢測檔案名稱衝突', async () => {
      ...
      const projectCode = 'TEST001'
      const albumName = '工程照片'
      const file = new File(['test content'], 'existing.jpg', { type: 'image/jpeg' })
      ...
      expect(result.error?.conflictData).toEqual({
        existingPath: path.join(testBaseDir, projectCode, albumName, 'existing.jpg'),
    ...
    it('應該拒絕不安全的檔案名稱', async () => {
      ...
      const projectCode = 'TEST001'
      const albumName = '工程照片'
      const file = new File(['test content'], '../../../malicious.exe', { type: 'application/octet-stream' })
  ...
  describe('deleteFile', () => {
    ...
    it('應該處理檔案不存在的情況', async () => {
      // Arrange
      const filePath = path.join(testBaseDir, 'TEST001', '相簿', 'nonexistent.jpg')
  ...
  describe('resolveFileConflict', () => {
    it('應該產生重新命名的檔案路徑', async () => {
      // Arrange
      const originalPath = path.join(testBaseDir, 'TEST001', '相簿', 'test.jpg')
  ...
  describe('ensureDirectoryExists', () => {
    it('應該建立不存在的目錄', async () => {
      ...
      // Assert
      expect(result.success).toBe(true)
      expect(mockFs.mkdir).toHaveBeenCalledWith(directoryPath, { recursive: true })
    })

    it('應該跳過已存在的目錄', async () => {
      // Arrange
      const directoryPath = path.join(testBaseDir, 'EXISTING_PROJECT', '現有相簿')
