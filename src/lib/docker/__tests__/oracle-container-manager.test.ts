...
describe('OracleContainerManager', () => {
  ...
  describe('容器狀態檢查', () => {
    it('應該正確檢查容器運行狀態', async () => {
      ...
      vi.mocked(require('child_process').exec).mockImplementation(
        (cmd, callback) => {
      ...
      vi.mocked(require('child_process').exec).mockImplementation(
        (cmd, callback) => {
          if (cmd.includes('docker exec')) {
            ...
            callback(null, {
              stdout: 'Connected to Oracle Database 21c Express Edition',
              stderr: '',
            });
          }
        }
      ...
      vi.mocked(require('child_process').exec).mockImplementation(
        (cmd, callback) => {
          if (cmd.includes('docker exec')) {
            ...
            callback(null, {
              stdout: 'Oracle instance is starting up',
              stderr: '',
            });
          }
        }
...
2025-01-23 10:02:00 [INFO] Database opened
      ...
      expect(logs[0].message).toContain(
        'Oracle Database 21c Express Edition starting'
      );
