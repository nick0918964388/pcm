...
describe('Oracle監控和診斷測試', () => {
  ...
  beforeEach(() => {
    ...
    mockConnection.execute.mockImplementation((query: string) => {
      ...
      if (query.includes('dba_data_files')) {
        return Promise.resolve({
          rows: [['USERS', 1024, 800, 224, 78.13, 'ONLINE', 1, 2048]],
        });
      }
      ...
      if (query.includes('v$lock') && !query.includes('w.sid')) {
        return Promise.resolve({
          rows: [[123, 'TM', '3', 'TEST_TABLE', 'TABLE', 0]],
        });
      }
      if (query.includes('v$lock') && query.includes('w.sid')) {
        return Promise.resolve({
      ...
      if (query.includes('v$archive_dest_status')) {
        return Promise.resolve({
          rows: [['LOG_ARCHIVE_DEST_1', 'VALID', null, 5000]],
        });
      }
  ...
  describe('RED: NLS配置管理', () => {
    it('應該檢查NLS(國際化)設定', async () => {
      ...
      // 檢查中文環境支援
      expect(['AL32UTF8', 'UTF8', 'ZHS16GBK', 'ZHT32EUC']).toContain(
        nlsSettings.characterSet
      );
    });
    ...
    it('應該驗證字符集兼容性', async () => {
      ...
      // Act
      const compatibility =
        await oracleMonitor.validateCharacterSetCompatibility(expectedCharset);
  ...
  describe('RED: 監控配置和通知', () => {
    ...
    it('應該生成監控報告', async () => {
      ...
      expect(report.summary.overallHealth).toMatch(
