...
// 創建排班 Schema
export const createDutyScheduleSchema = z
  .object({
    projectId: z.string().uuid('無效的專案 ID'),
    personId: z.string().uuid('無效的人員 ID'),
    dutyDate: z
      .string()
    ...
    notes: z.string().max(500, '備註長度不能超過 500 個字符').optional(),
  })
  .refine(
    data => {
...
export const setReplacementSchema = z.object({
  replacementPersonId: z.string().uuid('無效的代班人員 ID'),
  reason: z
    .string()
    .min(1, '代班原因不能為空')
...
export const batchCreateScheduleSchema = z.object({
  schedules: z
    .array(createDutyScheduleSchema)
    .min(1, '至少需要一個排班記錄')
...
export const dutyScheduleQuerySchema = z.object({
  ...
  sortBy: z
    .enum(['duty_date', 'shift_type', 'status', 'urgency_level', 'created_at'])
    .optional()
    .default('duty_date'),
  ...
  urgencyLevels: z
    .union([urgencyLevelEnum, z.array(urgencyLevelEnum)])
    .optional(),
...
export const createVendorSchema = z
  .object({
    ...
    email: z
      ...
      .optional(),
    address: z.string().max(300, '地址長度不能超過 300 個字符').optional(),
    contractStart: z
      .string()
...
export const vendorQuerySchema = z.object({
  ...
  pageSize: z.coerce.number().min(1).max(100).optional().default(20),
  sortBy: z
    .enum(['name', 'type', 'status', 'rating', 'contract_end', 'created_at'])
    .optional()
    .default('name'),
...
export const createDutyPersonSchema = z.object({
  ...
  employeeId: z.string().max(50, '員工編號長度不能超過 50 個字符').optional(),
  phone: z
    .string()
    .regex(/^[\d\-\+\(\)\s]+$/, '請輸入有效的電話號碼')
    .max(20, '電話號碼長度不能超過 20 個字符')
  ...
  email: z
    ...
    .max(100, '信箱長度不能超過 100 個字符')
    .optional(),
  position: z.string().max(100, '職位名稱長度不能超過 100 個字符').optional(),
...
export const updateVendorRatingSchema = z.object({
  rating: z.number().min(1, '評分最小值為 1').max(5, '評分最大值為 5'),
  comment: z.string().max(300, '評價內容長度不能超過 300 個字符').optional(),
});
...
export const batchScheduleOperationSchema = z.object({
  scheduleIds: z
    .array(z.string().uuid('無效的排班 ID'))
    .min(1, '至少需要選擇一個排班記錄'),
  action: z.enum(['approve', 'cancel', 'update_status', 'assign_replacement']),
  data: z.record(z.any()).optional(),
  reason: z.string().max(200, '操作原因長度不能超過 200 個字符').optional(),
});

// 導出所有 Schema 類型
export type CreateDutyScheduleRequest = z.infer<
  typeof createDutyScheduleSchema
>;
export type UpdateDutyScheduleRequest = z.infer<
...
export type BatchCreateScheduleRequest = z.infer<
  typeof batchCreateScheduleSchema
>;
...
export type UpdateVendorRatingRequest = z.infer<
  typeof updateVendorRatingSchema
>;
export type RenewVendorContractRequest = z.infer<
  typeof renewVendorContractSchema
>;
export type BatchScheduleOperationRequest = z.infer<
