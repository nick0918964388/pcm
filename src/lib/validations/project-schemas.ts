...
export const createProjectSchema = z
  .object({
    ...
    managerId: z.string().uuid('無效的專案經理 ID'),
...
export const updateProjectSchema = z.object({
  ...
  endDate: z
    ...
    .transform(val => (val ? new Date(val) : undefined)),
  budget: z.number().min(0, '預算不能為負數').optional(),
  managerId: z.string().uuid('無效的專案經理 ID').optional(),
  progress: z
    .number()
    .min(0, '進度不能為負數')
    .max(100, '進度不能超過 100%')
...
// 創建 WBS 項目 Schema
export const createWBSSchema = z
  .object({
    parentId: z.string().uuid('無效的父節點 ID').optional(),
    wbsCode: z
      .string()
      .min(1, 'WBS 代碼不能為空')
    ...
    assignedTo: z.string().uuid('無效的負責人 ID').optional(),
    estimatedHours: z.number().min(0, '預估工時不能為負數').optional(),
    startDate: z
      .string()
      .datetime('請輸入有效的開始日期')
      .optional()
    ...
    endDate: z
      ...
      .transform(val => (val ? new Date(val) : undefined)),
    dependencies: z
      .array(z.string().uuid('無效的依賴項目 ID'))
      .optional()
      .default([]),
    metadata: z.record(z.any()).optional(),
  })
...
export const updateWBSSchema = z.object({
  ...
  assignedTo: z.string().uuid('無效的負責人 ID').optional(),
  estimatedHours: z.number().min(0, '預估工時不能為負數').optional(),
  actualHours: z.number().min(0, '實際工時不能為負數').optional(),
  startDate: z
    .string()
    .datetime('請輸入有效的開始日期')
    .optional()
  ...
  completionPercentage: z
    ...
    .max(100, '完成度不能超過 100%')
    .optional(),
  dependencies: z.array(z.string().uuid('無效的依賴項目 ID')).optional(),
  metadata: z.record(z.any()).optional(),
});

// 移動 WBS 項目 Schema
export const moveWBSSchema = z.object({
  newParentId: z.string().uuid('無效的新父節點 ID').optional(),
});
...
export const batchWBSOperationSchema = z.object({
  wbsIds: z
    .array(z.string().uuid('無效的 WBS 項目 ID'))
