...
export const registerSchema = z
  .object({
    ...
    firstName: z.string().max(50, '姓名長度不能超過 50 個字符').optional(),
    lastName: z.string().max(50, '姓名長度不能超過 50 個字符').optional(),
  })
...
export const resetPasswordSchema = z
  .object({
    ...
    newPassword: passwordSchema,
    confirmPassword: z.string(),
    resetToken: z.string().min(1, '重設密碼 Token 不能為空'),
  })
...
export const createRoleSchema = z.object({
  ...
  description: z.string().max(200, '描述長度不能超過 200 個字符').optional(),
  permissions: z.array(z.string()).min(1, '至少需要一個權限'),
});
...
export const assignRoleSchema = z.object({
  userIds: z
    .array(z.string().uuid('無效的用戶 ID'))
    .min(1, '至少需要選擇一個用戶'),
  roleIds: z
    .array(z.string().uuid('無效的角色 ID'))
    .min(1, '至少需要選擇一個角色'),
});
...
export const batchUserOperationSchema = z.object({
  ...
  reason: z.string().max(200, '原因描述不能超過 200 個字符').optional(),
});

// 驗證郵箱 Schema
export const verifyEmailSchema = z.object({
  token: z.string().min(1, '驗證 Token 不能為空'),
  email: z.string().email('請輸入有效的信箱地址'),
});
...
export type BatchUserOperationRequest = z.infer<
  typeof batchUserOperationSchema
>;
