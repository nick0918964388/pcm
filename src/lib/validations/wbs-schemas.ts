...
// WBS 優先級
export const WbsPriorityEnum = z.enum(['low', 'medium', 'high', 'critical']);
...
export const BaseWbsSchema = z.object({
  ...
  description: z.string().max(1000, '描述不可超過1000字').optional(),
...
export const QueryWbsSchema = z
  ...
  .superRefine((data, ctx) => {
    // 驗證進度範圍
    if (
      data.progressMin !== undefined &&
      data.progressMax !== undefined &&
      data.progressMin > data.progressMax
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
// WBS 進度更新 Schema
export const UpdateWbsProgressSchema = z
  .object({
    progressPercent: z
      .number()
      .min(0, '進度百分比不可小於0')
...
export const WbsStatsQuerySchema = z.object({
  groupBy: z
    .enum(['status', 'type', 'priority', 'assignee', 'level'])
    .default('status'),
...
// WBS 依賴關係 Schema
export const WbsDependencySchema = z
  .object({
