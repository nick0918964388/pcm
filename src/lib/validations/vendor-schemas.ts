...
export const VendorStatusSchema = z.enum(['啟用', '停用', '待審核', '暫停']);
...
export const CreateVendorSchema = z
  .object({
    ...
    short_name: z.string().max(100, '廠商簡稱不能超過 100 個字元').optional(),

    type: VendorTypeSchema,

    status: VendorStatusSchema.default('active'),

    unified_number: z.string().max(20, '統一編號不能超過 20 個字元').optional(),
    ...
    address: z.string().max(300, '地址不能超過 300 個字元').optional(),
...
export const ContactStatusSchema = z.enum(['啟用', '停用']);
