...
export class EnvironmentConfig extends EventEmitter {
  ...
  private static loadOracleConfig(): OracleEnvironmentConfig {
    ...
    const oracle: OracleConfig = {
      ...
      password: process.env.ORACLE_PASSWORD ||
                ...
                'oracle123',
      poolMin: parseInt(process.env.ORACLE_POOL_MIN || fileConfig?.oracle?.poolMin || (envInfo.isProduction ? '5' : '1')),
      poolMax: parseInt(process.env.ORACLE_POOL_MAX || fileConfig?.oracle?.poolMax || (envInfo.isProduction ? '50' : '10')),
      poolIncrement: parseInt(process.env.ORACLE_POOL_INCREMENT || fileConfig?.oracle?.poolIncrement || '1'),
      poolTimeout: parseInt(process.env.ORACLE_POOL_TIMEOUT || fileConfig?.oracle?.poolTimeout || '60'),
  ...
  static getConfigFilePath(): string {
    const env = process.env.NODE_ENV || 'development'
    const configFileName = env === 'production' ? 'config.production.json' : 'config.development.json'
  ...
  static getSafeConfig(): OracleEnvironmentConfig {
    ...
    return {
      ...
      security: {
        ...config.security,
        walletPassword: config.security.walletPassword ? '***HIDDEN***' : undefined
      }
    }
  }
   ...
   * 比較配置變更
   */
  private static getConfigChanges(oldConfig: OracleEnvironmentConfig, newConfig: OracleEnvironmentConfig): Record<string, { old: any; new: any }> {
