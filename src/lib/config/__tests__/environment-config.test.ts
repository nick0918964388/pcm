...
import { EnvironmentConfig, ConfigValidationError } from '../environment-config'
...
describe('環境配置管理測試', () => {
  ...
  describe('RED: Oracle環境配置載入', () => {
    it('應該載入預設的Oracle開發環境配置', () => {
      ...
      // Assert
      expect(config.oracle.connectString).toBe(expectedConfig.oracle.connectString)
    ...
    it('應該支援環境變數覆蓋Oracle配置', () => {
      ...
      // Assert
      expect(config.oracle.connectString).toBe('prod-oracle.example.com:1521/PROD')
  ...
  describe('RED: 配置檔案支援', () => {
    it('應該載入config.json配置檔案', () => {
      ...
      vi.spyOn(EnvironmentConfig, 'loadConfigFile').mockReturnValue(mockConfigFile)
  ...
  describe('RED: 錯誤處理和診斷', () => {
    ...
    it('應該提供配置修復建議', () => {
      ...
      // Act
      const suggestions = EnvironmentConfig.getConfigSuggestions(incompleteConfig)
