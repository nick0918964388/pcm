...
export class DatabaseAbstraction implements IDatabaseAbstraction {
  ...
  private wrapDatabaseError(
    ...
    if (originalError.message.includes('relation') && originalError.message.includes('does not exist')) {
      code = 'TABLE_NOT_FOUND'
      message = 'Table or relation does not exist'
    }

    // Connection timeout errors
    if (originalError.message.includes('timeout') || originalError.message.includes('TIMEOUT')) {
      code = 'TIMEOUT_ERROR'
