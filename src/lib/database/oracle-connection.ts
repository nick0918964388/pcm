...
export class OracleConnectionManager {
  ...
  async executeOne<T>(
    sql: string,
    binds: BindParameters = {}
  ): Promise<Result<T | null, QueryError>> {
    ...
    if (!this.isInitialized || !this.pool) {
      return {
        ...
        error: new TransactionError(
          'Oracle connection manager not initialized'
        ),
      };
    }
  ...
  getPoolStatus(): PoolMetrics {
    ...
    return {
      ...
      idleConnections:
        (this.pool.connectionsOpen || 0) - (this.pool.connectionsInUse || 0),
    ...
    return {
      ...
      averageConnections:
        activeConnections.length > 0
          ? activeConnections.reduce((sum, count) => sum + count, 0) /
  ...
  handleOracleError(error: Error): {
    ...
    const suggestedAction =
      suggestions[errorCode] || 'Check Oracle error documentation';
  ...
  private updateQueryMetrics(executionTime: number, hasError: boolean): void {
    ...
    const totalTime =
      this.queryMetrics.averageExecutionTime *
        (this.queryMetrics.totalQueries - 1) +
      executionTime;
    this.queryMetrics.averageExecutionTime =
      totalTime / this.queryMetrics.totalQueries;
    ...
    this.queryMetrics.errorRate =
      this.connectionFailures / this.queryMetrics.totalQueries;
  }
}
