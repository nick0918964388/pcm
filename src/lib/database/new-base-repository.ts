...
export class NewBaseRepository<T extends AuditFields>
  implements IRepository<T>
{
  ...
  async findById(id: string): Promise<T | null> {
    try {
      ...
      throw new Error(
        `Failed to find ${this.tableName} by id ${id}: ${(error as Error).message}`
      );
    }
  }
  ...
  async findMany(criteria: SearchCriteria): Promise<PaginatedResult<T>> {
    try {
      ...
      const [totalResult, dataResult] = await Promise.all([
        this.connection.queryOne<{ count: number }>(
          countQuery.sql,
          countQuery.params
        ),
      ...
      throw new Error(
        `Failed to find ${this.tableName}: ${(error as Error).message}`
      );
    }
  }
  ...
  async create(entity: Omit<T, 'id' | 'createdAt' | 'updatedAt'>): Promise<T> {
    ...
    try {
      ...
      throw new Error(
        `Failed to create ${this.tableName}: ${(error as Error).message}`
      );
    }
  }
  ...
  async update(id: string, updates: Partial<T>): Promise<T> {
    ...
    try {
      ...
      throw new Error(
        `Failed to update ${this.tableName}: ${(error as Error).message}`
      );
    }
  }
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to delete ${this.tableName}: ${(error as Error).message}`
      );
    }
  }
  ...
  private buildCountQuery(criteria: SearchCriteria): {
    sql: string;
    params: any[];
  } {
