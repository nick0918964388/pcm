...
describe('OracleConnectionManager', () => {
  ...
  describe('初始化Oracle連線管理器', () => {
    it('應該成功初始化連線池', async () => {
      ...
      vi.mocked(await import('oracledb')).default.createPool.mockResolvedValue(
        mockPool
      );
      ...
      expect(
        vi.mocked(await import('oracledb')).default.createPool
    ...
    it('應該在連線失敗時返回錯誤', async () => {
      ...
      vi.mocked(await import('oracledb')).default.createPool.mockRejectedValue(
        error
      );
  ...
  describe('執行查詢操作', () => {
    ...
    it('應該成功執行查詢並返回結果', async () => {
      ...
      const result = await connectionManager.executeQuery<{
        id: number;
        name: string;
      }>('SELECT id, name FROM projects WHERE id = :id', { id: 1 });
  ...
  describe('交易管理', () => {
    ...
    it('應該成功執行交易並提交', async () => {
      ...
      // When
      const result =
        await connectionManager.executeTransaction(transactionCallback);
