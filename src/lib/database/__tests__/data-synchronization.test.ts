...
describe('Data Synchronization and Consistency', () => {
  ...
  describe('DataSynchronizer', () => {
    it('should synchronize data between PostgreSQL and Oracle', async () => {
      ...
      // When
      const result = await synchronizer.synchronizeTable('projects', 'postgresql-to-oracle')
    ...
    it('should detect and handle data conflicts', async () => {
      // Given
      mockPgConnection.query.mockResolvedValueOnce({
        rows: [{ id: 'proj-1', name: 'Project Updated PG', updated_at: '2025-01-23 10:00:00' }]
      })
    ...
    it('should perform incremental synchronization', async () => {
      ...
      mockPgConnection.query.mockResolvedValueOnce({
        rows: [{ id: 'proj-3', name: 'New Project', created_at: '2025-01-23 10:00:00' }]
      })
      ...
      // When
      const result = await synchronizer.incrementalSync('projects', lastSyncTime)
    ...
    it('should handle synchronization errors gracefully', async () => {
      // Given
      mockOracleConnection.execute.mockRejectedValueOnce(new Error('Oracle connection failed'))
  ...
  describe('ConsistencyChecker', () => {
    it('should check data consistency between databases', async () => {
      // Given
      mockPgConnection.query.mockResolvedValueOnce({ rows: [{ count: '100' }] })
  ...
  describe('ConflictResolver', () => {
    it('should resolve conflicts using timestamp strategy', async () => {
      ...
      const conflict = {
        ...
        postgresqlData: { id: 'proj-1', name: 'Project PG', updated_at: '2025-01-23 10:00:00' },
        oracleData: { id: 'proj-1', name: 'Project Oracle', updated_at: '2025-01-23 10:30:00' },
  ...
  describe('DataComparisonEngine', () => {
    it('should compare record-level data between databases', async () => {
      ...
      const postgresRecord = { id: 'proj-1', name: 'Project 1', status: 'active' }
      const oracleRecord = { id: 'proj-1', name: 'Project One', status: 'active' }
    ...
    it('should generate detailed comparison reports', async () => {
      ...
      // When
      const report = await engine.generateComparisonReport(['projects', 'users'])
