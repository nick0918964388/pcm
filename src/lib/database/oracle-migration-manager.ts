...
export class OracleMigrationManager {
  ...
  private async ensureMigrationTable(connection: oracledb.Connection): Promise<void> {
  ...
  private async isScriptExecuted(connection: oracledb.Connection, scriptName: string): Promise<boolean> {
    const result = await connection.execute(`
  ...
  private async executeScript(connection: oracledb.Connection, scriptFile: string): Promise<void> {
    const scriptsDir = path.join(process.cwd(), 'database', 'oracle')
  ...
  private async recordScriptExecution(connection: oracledb.Connection, scriptName: string): Promise<void> {
    await connection.execute(`
...
export class OracleTestDataManager {
  ...
  async loadTestData(): Promise<TestDataResult> {
    ...
    try {
      ...
      const testDataScript = path.join(process.cwd(), 'database', '04-initial-data.sql')
  ...
  private async getUserTables(connection: oracledb.Connection): Promise<string[]> {
    const result = await connection.execute(`
      SELECT table_name
      FROM user_tables
...
export class OracleMaintenanceTools {
    ...
    try {
      ...
      for (const table of tables) {
        try {
          ...
          console.warn(`Failed to update statistics for table ${table}:`, error)
        }
      }
  ...
  async rebuildIndexes(): Promise<MaintenanceResult> {
    ...
    try {
      ...
      const indexes = (result.rows || []).map(row => (row as any[])[0] as string)
