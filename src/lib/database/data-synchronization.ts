...
export class DataSynchronizer {
  ...
  async synchronizeTable(
    ...
    try {
      ...
      if (
        direction === 'postgresql-to-oracle' ||
      ...
      if (
        direction === 'oracle-to-postgresql' ||
        direction === 'bidirectional'
      ) {
  ...
  async incrementalSync(
    ...
    try {
      // 獲取PostgreSQL中的變更
      const pgChanges = await this.getChangesFromPostgres(
        tableName,
        lastSyncTime
      );

      // 獲取Oracle中的變更
      const oracleChanges = await this.getChangesFromOracle(
        tableName,
        lastSyncTime
      );
  ...
  private async syncFromPostgresToOracle(
    tableName: string
  ): Promise<{ recordsSynchronized: number; conflicts: DataConflict[] }> {
  ...
  private async syncFromOracleToPostgres(
    tableName: string
  ): Promise<{ recordsSynchronized: number; conflicts: DataConflict[] }> {
    let oracleConnection: oracledb.Connection | undefined;
  ...
  private async getChangesFromPostgres(
    tableName: string,
    since: Date
  ): Promise<any[]> {
  ...
  private async getChangesFromOracle(
    tableName: string,
    since: Date
  ): Promise<any[]> {
    let oracleConnection: oracledb.Connection | undefined;
    ...
    try {
      ...
      return (
        result.rows?.map(row =>
          this.mapOracleRowToObject(row as any[], tableName)
        ) || []
      );
    } finally {
      if (oracleConnection) {
  ...
  private async applyChangesToOracle(
    change: any
  ): Promise<{ conflict?: DataConflict }> {
    let oracleConnection: oracledb.Connection | undefined;
    ...
    try {
      ...
      if (exists) {
        ...
        if (oracleRecord.rows && oracleRecord.rows.length > 0) {
          const oracleData = this.mapOracleRowToObject(
            oracleRecord.rows[0] as any[],
            change.tableName
          );
          const oracleUpdatedAt = new Date(
            oracleData.updated_at || oracleData.created_at
          );
  ...
  private async applyChangesToPostgres(
    change: any
  ): Promise<{ conflict?: DataConflict }> {
    try {
      // 檢查PostgreSQL中是否存在該記錄
  ...
  private async updateOracleRecord(
  ...
  private async insertOracleRecord(
    connection: oracledb.Connection,
    tableName: string,
    record: any
  ): Promise<void> {
  ...
  private async updatePostgresRecord(
    tableName: string,
    record: any
  ): Promise<void> {
    // 構建動態UPDATE語句
  ...
  private async insertPostgresRecord(
    tableName: string,
    record: any
  ): Promise<void> {
    // 構建動態INSERT語句
...
export class ConsistencyChecker {
  ...
  async checkTableConsistency(tableName: string): Promise<ConsistencyResult> {
    ...
    try {
      // 獲取PostgreSQL記錄數
      const pgCountResult = await this.pgPool.query(
  ...
  async validateAllTables(): Promise<ValidationReport> {
    ...
    const totalDiscrepancies = tableResults.reduce(
  ...
  async findMissingRecords(tableName: string): Promise<MissingRecordsResult> {
    ...
    try {
      // 獲取PostgreSQL的所有ID
      const pgIdsResult = await this.pgPool.query(
      ...
      // 獲取Oracle的所有ID
      oracleConnection = await oracledb.getConnection(this.oracleConfig);
      const oracleIdsResult = await oracleConnection.execute(
        `SELECT id FROM ${tableName}`
      );
      const oracleIds = (oracleIdsResult.rows || []).map(
        row => (row as any[])[0]
      );
...
export class ConflictResolver {
  ...
  async resolveConflict(
    ...
    try {
      switch (conflict.resolution) {
        ...
        default:
          throw new Error(
            `Unknown resolution strategy: ${conflict.resolution}`
          );
      }
    } catch (error) {
      return {
  ...
  private resolveByTimestamp(conflict: DataConflict): ConflictResolutionResult {
    const pgTimestamp = new Date(
      conflict.postgresqlData.updated_at || conflict.postgresqlData.created_at
    );
    const oracleTimestamp = new Date(
      conflict.oracleData.updated_at || conflict.oracleData.created_at
    );

    const chosenSource =
      oracleTimestamp > pgTimestamp ? 'oracle' : 'postgresql';
    const resolvedData =
      chosenSource === 'oracle' ? conflict.oracleData : conflict.postgresqlData;
  ...
  private resolveBySource(
    ...
    const resolvedData =
      source === 'postgresql' ? conflict.postgresqlData : conflict.oracleData;
  ...
  private resolveByCustomRule(
    ...
    const resolvedData = customRule(
      conflict.postgresqlData,
      conflict.oracleData
    );
    ...
    if (
      JSON.stringify(resolvedData) === JSON.stringify(conflict.postgresqlData)
    ) {
      chosenSource = 'postgresql';
    } else if (
      JSON.stringify(resolvedData) === JSON.stringify(conflict.oracleData)
    ) {
      chosenSource = 'oracle';
    }
...
export class SyncScheduler {
  ...
  async scheduleSync(config: ScheduleConfig): Promise<ScheduleResult> {
    try {
      ...
      if (
        !['manual', 'real-time', 'hourly', 'daily', 'weekly'].includes(
          config.interval
        )
      ) {
...
export class DataComparisonEngine {
  ...
  async compareTableSchemas(tableName: string): Promise<SchemaComparison> {
    ...
    try {
      ...
      const pgColumns = new Map(
        pgSchemaResult.rows.map(row => [row.column_name, row])
      );
    ...
    for (const tableName of tableNames) {
      try {
        ...
        const dataConsistency =
          await consistencyChecker.checkTableConsistency(tableName);
        ...
        totalIssues +=
          schemaCompatibility.differences.length +
          dataConsistency.discrepancies.length;
  ...
  private areTypesCompatible(pgType: string, oracleType: string): boolean {
    ...
    return compatibleOracleTypes.some(type =>
      oracleType.toUpperCase().includes(type)
    );
  }
}
