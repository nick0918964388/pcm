...
export interface WBSValidationError {
  /** 錯誤類型 */
  type: 'CIRCULAR_DEPENDENCY' | 'INVALID_CODE' | 'MISSING_PARENT' | 'INVALID_DATE' | 'INVALID_ASSIGNEE'
  /** 錯誤訊息 */
  message: string
  /** 相關節點 ID */
  nodeId: string
...
export const flattenWBSTree = (
  ...
  const traverse = (nodes: WBSItem[], level: number = 0, path: string[] = [], parent?: WBSItem) => {
...
export const filterWBSTree = (
  ...
  return tree
    .map(filterNode)
    .filter((node): node is WBSItem => node !== null)
}
...
export const findWBSNodePath = (
  ...
  const findPath = (nodes: WBSItem[], targetId: string, currentPath: WBSItem[] = []): WBSItem[] | null => {
    for (const node of nodes) {
...
export const generateWBSCodes = (
  ...
  const generateCode = (level: number, index: number, parentCode?: string): string => {
    ...
    if (customMapper) {
      ...
      switch (format) {
        case 'alphanumeric':
          levelCode = index < 26 ? String.fromCharCode(65 + index) : `A${index - 25}`
          break
        case 'numeric':
        default:
  ...
  const processNodes = (nodes: WBSItem[], level: number = 0, parentCode?: string): WBSItem[] => {
    return nodes.map((node, index) => ({
      ...node,
      code: generateCode(level, index, parentCode),
      children: node.children 
        ? processNodes(node.children, level + 1, generateCode(level, index, parentCode))
        : []
    }))
  }
 ...
 * @returns 循環依賴檢查結果
 */
export const checkCircularDependencies = (tree: WBSItem[]): WBSValidationError[] => {
...
export const validateWBSTree = (tree: WBSItem[]): WBSValidationResult => {
  ...
  warnings.push(...dateErrors.filter(e => e.severity === 'warning') as WBSValidationWarning[])
