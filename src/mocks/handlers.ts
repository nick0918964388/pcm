...
function parseSearchParams(url: URL) {
  const params = {
    ...
    // Date range filters
    dateFrom: url.searchParams.get('dateFrom') ? new Date(url.searchParams.get('dateFrom')!) : null,
    dateTo: url.searchParams.get('dateTo') ? new Date(url.searchParams.get('dateTo')!) : null,
    ...
    minProgress: url.searchParams.get('minProgress') ? parseInt(url.searchParams.get('minProgress')!, 10) : null,
    maxProgress: url.searchParams.get('maxProgress') ? parseInt(url.searchParams.get('maxProgress')!, 10) : null,
  }
 ...
 */
function applyStatusFilter(projects: Project[], statuses: ProjectStatus[]): Project[] {
  if (!statuses.length) {
    return projects
  }
...
function serializeProjectDates(project: Project): Project {
  return {
    ...
    actualStartDate: project.actualStartDate ? new Date(project.actualStartDate).toISOString() : undefined,
    actualEndDate: project.actualEndDate ? new Date(project.actualEndDate).toISOString() : undefined,
    createdAt: new Date(project.createdAt).toISOString(),
    updatedAt: new Date(project.updatedAt).toISOString(),
    lastAccessDate: project.lastAccessDate ? new Date(project.lastAccessDate).toISOString() : undefined,
    ...
    permissions: project.permissions.map(permission => ({
      ...
      expiresAt: permission.expiresAt ? new Date(permission.expiresAt).toISOString() : undefined,
    })),
  } as Project
}
...
const getProjectsHandler = http.get('/api/projects', async ({ request }) => {
  ...
  try {
    ...
    // Sort projects
    filteredProjects = sortProjects(filteredProjects, params.sortBy, params.sortOrder)

    // Apply pagination
    const result = paginateProjects(filteredProjects, params.page, params.limit)
    ...
    const responseWithMeta = {
      ...
      metadata: {
        ...
        appliedFilters: {
          ...
          progressRange: params.minProgress !== null || params.maxProgress !== null ? {
    ...
    return HttpResponse.json(
      createApiResponse(false, null, '獲取專案列表時發生錯誤', 'INTERNAL_ERROR'),
      { status: 500 }
    )
  }
})
...
const getProjectHandler = http.get('/api/projects/:id', async ({ params }) => {
  ...
  try {
    ...
    return HttpResponse.json(
      createApiResponse(false, null, '獲取專案詳情時發生錯誤', 'INTERNAL_ERROR'),
      { status: 500 }
    )
  }
})
 ...
 */
const recordProjectAccessHandler = http.post('/api/projects/:id/access', async ({ params }) => {
  await addRealisticDelay()
  ...
  try {
    ...
    return HttpResponse.json(
      createApiResponse(false, null, '記錄專案存取時發生錯誤', 'INTERNAL_ERROR'),
      { status: 500 }
    )
  }
})
...
const getProjectSummaryHandler = http.get('/api/projects/summary', async () => {
  ...
  try {
    const response = createApiResponse(true, {
      ...
      attentionRequired: mockProjects
        .filter(p => 
          ...
          (p.status === ProjectStatus.IN_PROGRESS && p.progress < 50 && 
           new Date(p.endDate).getTime() - new Date().getTime() < 90 * 24 * 60 * 60 * 1000)
        )
        .slice(0, 5),
    })
    ...
    return HttpResponse.json(
      createApiResponse(false, null, '獲取專案摘要時發生錯誤', 'INTERNAL_ERROR'),
      { status: 500 }
    )
  }
})
 ...
 */
const getProjectStatisticsHandler = http.get('/api/projects/statistics', async ({ request }) => {
  await addRealisticDelay()
  ...
  try {
    ...
    const statistics = {
      ...
      timeRange,
      groupBy,
      breakdown: groupBy === 'status' ? mockProjectSummary.projectsByStatus : mockProjectSummary.projectsByType,
    }
    ...
    return HttpResponse.json(
      createApiResponse(false, null, '獲取專案統計時發生錯誤', 'INTERNAL_ERROR'),
      { status: 500 }
    )
  }
})
 ...
 */
const getProjectProgressUpdatesHandler = http.get('/api/projects/progress-updates', async ({ request }) => {
  await addRealisticDelay()

  try {
    const url = new URL(request.url)
    const projectIds = url.searchParams.getAll('projectId')
    const since = url.searchParams.get('since') ? new Date(url.searchParams.get('since')!) : new Date(Date.now() - 60 * 60 * 1000) // Last hour
    ...
    return HttpResponse.json(
      createApiResponse(false, null, '獲取進度更新時發生錯誤', 'INTERNAL_ERROR'),
      { status: 500 }
    )
  }
})
