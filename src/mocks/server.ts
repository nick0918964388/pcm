...
function simulateRealtimeUpdates(subscription: RealtimeSubscription): void {
  ...
  const generateUpdates = () => {
    try {
      ...
      if (serverConfig.enableLogging) {
        console.log(
          `[MSW Server] 模擬即時更新: ${updates.length} 個更新`,
          updates
        );
      }
  ...
  if (serverConfig.enableLogging) {
    console.log(
      `[MSW Server] 啟動即時更新模擬，間隔: ${interval}ms，專案: ${projectIds.join(', ')}`
    );
  }
}
  ...
  try {
    ...
    nodeServer.events.on('request:match', ({ request }) => {
      if (mergedConfig.development?.debug) {
        console.log(
          `[MSW Server] 匹配處理器: ${request.method} ${request.url}`
        );
      }
    });
    ...
    nodeServer.events.on('request:unhandled', ({ request }) => {
      if (mergedConfig.enableLogging) {
        console.warn(
          `[MSW Server] 未處理的請求: ${request.method} ${request.url}`
        );
      }
    });
...
async function initializeMSW(
  config: ServerConfig = {}
): Promise<SetupServer | SetupWorker> {
...
if (
  isDevelopment() &&
  serverConfig.development?.hotReload &&
 ...
 */
export { isBrowser, isNode, isDevelopment, isTesting };

/**
 * Configuration management
 */
export { serverConfig, defaultConfig };

/**
 * Type definitions
 */
export type { ServerConfig, RealtimeSubscription };
