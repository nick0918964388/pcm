...
export class EventBus {
  ...
  private eventStats: Map<string, { count: number; totalTime: number }> = new Map()
  ...
  async emitBatch(events: BatchEvent[]): Promise<void> {
    const promises = events.map(event => this.emit(event.eventName, event.data))
  ...
  getStatistics(): EventStatistics {
    ...
    return {
      ...
      eventTypes,
      averageProcessingTime: totalEventsProcessed > 0 ? totalProcessingTime / totalEventsProcessed : 0
    }
  }
  ...
  getSlowHandlers(thresholdMs: number): SlowHandler[] {
    ...
    return slowHandlers.sort((a, b) => b.avgProcessingTime - a.avgProcessingTime)
  }
  ...
  async cleanupProcessedEvents(): Promise<void> {
    ...
    if (this.processedEventIds.size > this.MAX_PROCESSED_EVENTS) {
      const sortedEntries = Array.from(this.processedEventIds.entries())
        .sort(([, a], [, b]) => a.getTime() - b.getTime())

      const toDelete = sortedEntries.slice(0, this.processedEventIds.size - this.MAX_PROCESSED_EVENTS)
    ...
    if (this.debugMode) {
      console.debug(`Cleaned up processed events. Remaining: ${this.processedEventIds.size}`)
    }
  }
