...
function transformProjectDates(project: any): Project {
  return {
    ...
    lastAccessDate: project.lastAccessDate
      ? new Date(project.lastAccessDate)
      : undefined,
    teamMembers:
    ...
    permissions:
      project.permissions?.map((permission: any) => ({
        ...permission,
...
function validateGetProjectsParams(
  ...
  if (pagination) {
    ...
    if (
      ...
      pagination.pageSize > API_CONFIG.MAX_PAGE_SIZE
    ) {
      throw new ValidationError('Invalid page size', {
        pageSize: [
          `每頁資料筆數必須介於 1 到 ${API_CONFIG.MAX_PAGE_SIZE} 之間`,
        ],
      });
    }
  }
  ...
  if (
    filters?.search &&
    filters.search.length < PROJECT_CONSTANTS.MIN_SEARCH_LENGTH
  ) {
    throw new ValidationError('Search term too short', {
      search: [
        `搜尋關鍵字至少需要 ${PROJECT_CONSTANTS.MIN_SEARCH_LENGTH} 個字元`,
      ],
    });
  }
...
class HttpClient {
  ...
  async request<T>(endpoint: string, config: RequestConfig): Promise<T> {
    ...
    for (
      let attempt = 0;
      attempt < (config.retries || API_CONFIG.RETRY_ATTEMPTS);
      attempt++
    ) {
      try {
...
export class ProjectApiService {
  ...
  async getProjectSummary(
    ...
    try {
      ...
      const response = await this.client.get<ApiResponse<ProjectStatistics>>(
        '/projects/summary',
        queryParams
      );
  ...
  async getProjectDashboard(): Promise<ApiResponse<ProjectDashboard>> {
    try {
      const response = await this.client.get<ApiResponse<ProjectDashboard>>(
        '/projects/dashboard'
      );
      ...
      const transformedData = {
        ...
        upcomingDeadlines: response.data.upcomingDeadlines.map(
          transformProjectDates
        ),
        attentionRequired: response.data.attentionRequired.map(
          transformProjectDates
        ),
      };
  ...
  async searchProjects(
    ...
    return this.getProjects({ search: query }, undefined, {
      page: 1,
      pageSize: Math.min(limit, API_CONFIG.MAX_PAGE_SIZE),
    });
  }
  ...
  > {
    try {
      const response =
        await this.client.get<ApiResponse<{ version: string; build: string }>>(
          '/version'
        );
...
export { ApiError, NetworkError, ValidationError, HttpStatus };
