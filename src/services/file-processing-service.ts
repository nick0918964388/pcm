...
export class FileProcessingService {
  ...
  async verifyFileIntegrity(
    file: File,
    expectedHash: string
  ): Promise<{ isValid: boolean; checksum: string }> {
  ...
  async generateThumbnails(
    ...
    for (const [sizeName, dimensions] of Object.entries(options.sizes)) {
      if (options.formats.includes('webp')) {
        srcSetEntries.push(
          `${thumbnails[`${sizeName}_webp`]} ${dimensions.width}w`
        );
      }
    }
  ...
  async performStorageCleanup(
    ...
    const usagePercent =
      (storageStatus.usedSpace / storageStatus.totalSpace) * 100;
  ...
  private async getStorageStatus(): Promise<{
    totalSpace: number;
    usedSpace: number;
