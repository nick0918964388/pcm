...
export class ProjectApiServiceExtension {
  ...
  async getWBSTree(projectId: string): Promise<WBSApiResponse<WBSItem[]>> {
    return this.client.get<WBSApiResponse<WBSItem[]>>(
  ...
  async getWBSItems(
    ...
    return this.client.get<WBSQueryResult>(
  ...
  async validateWBSStructure(
    projectId: string
  ): Promise<WBSApiResponse<WBSValidationResult>> {
   ...
   * 取得 WBS 統計資料
   */
  async getWBSStatistics(
    projectId: string
  ): Promise<WBSApiResponse<WBSStatistics>> {
...
export class ProjectStaffWBSApiService extends ProjectApiServiceExtension {
  ...
  async queryProjectMembers(
    ...
    if (options.filters) {
      ...
      if (options.filters.role?.length)
        params.append('role', options.filters.role.join(','));
      if (options.filters.skills?.length)
        params.append('skills', options.filters.skills.join(','));
      if (options.filters.workStatus?.length)
        params.append('workStatus', options.filters.workStatus.join(','));
      ...
      if (options.filters.workloadRange) {
        if (options.filters.workloadRange.min !== undefined)
          params.append(
            'workloadMin',
            options.filters.workloadRange.min.toString()
          );
        if (options.filters.workloadRange.max !== undefined)
          params.append(
            'workloadMax',
            options.filters.workloadRange.max.toString()
          );
      }
    }
  ...
  async searchProjectMembers(
    projectId: string,
    query: string
  ): Promise<StaffApiResponse<SearchSuggestion[]>> {
  ...
  async getProjectMemberStats(
    projectId: string
  ): Promise<StaffApiResponse<ProjectMemberStats>> {
  ...
  async getProjectMemberDetail(
    projectId: string,
    memberId: string
  ): Promise<StaffApiResponse<ProjectMemberExtended>> {
  ...
  ): Promise<ProjectMemberExtended> {
    const response = await this.client.post<
      StaffApiResponse<ProjectMemberExtended>
   ...
   * 批量操作專案成員
   */
  async bulkOperateProjectMembers(
    projectId: string,
    request: { operation: string; memberIds: string[]; data?: any }
  ): Promise<any> {
    return this.client.post(`/projects/${projectId}/members/bulk`, request);
  }
  ...
  async analyzeWorkload(projectId: string): Promise<ApiResponse<any>> {
    return this.client.get<ApiResponse<any>>(
      `/projects/${projectId}/analysis/workload`
    );
  }
   ...
   * 專案配置分析
   */
  async analyzeProjectConfiguration(
    projectId: string
  ): Promise<ApiResponse<any>> {
    return this.client.get<ApiResponse<any>>(
      `/projects/${projectId}/analysis/configuration`
    );
  }
