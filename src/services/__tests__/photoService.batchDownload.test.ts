...
describe('PhotoService - Batch Download (Task 5-2)', () => {
  ...
  describe('ZIP打包功能', () => {
    ...
    it('應該為每張照片在ZIP中建立正確的檔名', async () => {
      ...
      expect(mockJSZip.file).toHaveBeenCalledWith('photo1.jpg', expect.anything())
      expect(mockJSZip.file).toHaveBeenCalledWith('photo2.png', expect.anything())
    })
    ...
    it('應該處理重複檔名，自動重新命名', async () => {
      ...
      expect(mockJSZip.file).toHaveBeenCalledWith('photo1.jpg', expect.anything())
      expect(mockJSZip.file).toHaveBeenCalledWith('photo1_2.jpg', expect.anything())
    })
  })
  ...
  describe('大檔案處理', () => {
    ...
    it('應該在檔案總大小超過限制時建議分批下載', () => {
      // RED: 會失敗，邏輯未實作
      const totalSize = mockPhotos.reduce((sum, photo) => sum + photo.fileSize, 0)
  ...
  describe('錯誤處理', () => {
    ...
    it('應該在ZIP生成失敗時提供適當錯誤訊息', async () => {
      // RED: 會失敗，錯誤處理未實作
      mockJSZip.generateAsync.mockRejectedValue(new Error('ZIP generation failed'))

      await expect(
        photoService.downloadPhotos(mockPhotos)
  ...
  describe('效能考量', () => {
    it('應該限制同時下載的檔案數量', async () => {
      ...
      await photoService.downloadPhotos(manyPhotos, { concurrency: concurrencyLimit })
