...
describe('EnhancedPhotoService', () => {
  ...
  describe('Photo Upload Business Logic (Task 2.1)', () => {
    it('should separate file processing and metadata management', async () => {
      ...
      const mockMetadata = { width: 1920, height: 1080, capturedAt: new Date() }

      mockFileProcessingService.validateFile.mockResolvedValue({ isValid: true, errors: [] })
      ...
      mockDatabaseAbstraction.createUnitOfWork.mockReturnValue(mockUnitOfWork as any)

      const result = await photoService.uploadPhoto(mockFile, 'project-1', 'user-1', 'album-1')

      expect(mockFileProcessingService.validateFile).toHaveBeenCalledWith(mockFile)
      expect(mockFileProcessingService.extractMetadata).toHaveBeenCalledWith(mockFile)
      expect(mockFileProcessingService.processFileAsync).toHaveBeenCalledWith(mockFile, expect.any(Object))
    ...
    it('should implement circuit breaker on service errors', async () => {
      ...
      mockFileProcessingService.validateFile.mockRejectedValue(new Error('Service unavailable'))

      // First 5 failures should be attempted
      for (let i = 0; i < 5; i++) {
        await expect(photoService.uploadPhoto(mockFile, 'project-1', 'user-1'))
    ...
    it('should ensure data consistency through business rules', async () => {
      ...
      mockFileProcessingService.validateFile.mockResolvedValue({ isValid: true, errors: [] })
      mockFileProcessingService.extractMetadata.mockResolvedValue({ width: 1920, height: 1080 })
      ...
      const result = await photoService.uploadPhoto(mockFile, 'project-1', 'user-1')
    ...
    it('should implement business layer error handling and recovery', async () => {
      ...
      mockFileProcessingService.validateFile.mockRejectedValueOnce(new Error('Temporary failure'))
  ...
  describe('File Processing Service Layer (Task 2.2)', () => {
    it('should implement asynchronous file processing workflow', async () => {
      ...
      mockFileProcessingService.processFileAsync.mockImplementation(async (file, options) => {
      ...
      expect(mockFileProcessingService.processFileAsync).toHaveBeenCalledWith(mockFile, expect.objectContaining({
        async: true,
        generateThumbnails: true,
        compress: true
      }))

      expect(mockEventBus.emit).toHaveBeenCalledWith('file.processing.started', expect.any(Object))
      expect(result.success).toBe(true)
    })
    ...
    it('should support multi-size image processing for different devices', async () => {
      ...
      expect(mockFileProcessingService.generateThumbnails).toHaveBeenCalledWith(mockFile, expect.objectContaining({
        sizes: ['thumbnail', 'medium', 'large'],
        formats: ['webp', 'jpeg']
      }))
    ...
    it('should implement file integrity check and deduplication', async () => {
      ...
      const mockRepository = {
        findByField: jest.fn().mockResolvedValue([
      ...
      expect(mockRepository.findByField).toHaveBeenCalledWith('checksum', 'sha256:abc123def456')
      expect(result.success).toBe(true)
    ...
    it('should implement queue mechanism and parallel processing', async () => {
      ...
      mockFileProcessingService.processFileAsync.mockImplementation(async (file) => ({
      ...
      const result = await photoService.batchUploadPhotos(mockFiles, 'project-1', 'user-1', {
        concurrency: 3,
        onProgress: progressCallback
      })

      expect(mockFileProcessingService.processFileAsync).toHaveBeenCalledTimes(3)
    ...
    it('should implement automatic cleanup and tiered storage', async () => {
      ...
      jest.spyOn(photoService as any, 'getStorageInfo').mockResolvedValue(mockStorageInfo)
      jest.spyOn(photoService as any, 'cleanupOldFiles').mockResolvedValue({ deletedCount: 5, freedSpace: 1024 * 1024 * 1024 })
  ...
  describe('Event-Driven Architecture and Service Collaboration (Task 2.3)', () => {
    it('should establish lightweight event bus for service communication', async () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('photo.upload.started', expect.objectContaining({
        fileId: expect.any(String),
        projectId: 'project-1',
        userId: 'user-1'
      }))

      expect(mockEventBus.emit).toHaveBeenCalledWith('photo.upload.completed', expect.objectContaining({
    ...
    it('should implement service circuit breaker and degradation strategy', async () => {
      ...
      mockFileProcessingService.processFileAsync.mockRejectedValue(new Error('Service temporarily unavailable'))

      const mockFile = new File(['test'], 'test.jpg', { type: 'image/jpeg' })

      // Test circuit breaker activation
      const isCircuitOpen = await photoService.isCircuitBreakerOpen('fileProcessing')
      ...
      const isCircuitOpenAfterFailures = await photoService.isCircuitBreakerOpen('fileProcessing')
    ...
    it('should support idempotent event processing', async () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('photo.metadata.updated', expect.objectContaining({
        eventId: 'event-456'
      }))
    ...
    it('should implement service collaboration error handling and retry', async () => {
      ...
      expect(mockFileProcessingService.processFileAsync).toHaveBeenCalledTimes(2)
      expect(result.success).toBe(true)
