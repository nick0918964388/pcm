...
describe('EventBus', () => {
  ...
  describe('Event Patterns and Filtering', () => {
    ...
    it('should support event filtering', async () => {
      ...
      await eventBus.emit('photo.updated', { photoId: '1', projectId: 'project-1' })
      await eventBus.emit('photo.updated', { photoId: '2', projectId: 'project-2' })

      expect(handler).toHaveBeenCalledTimes(1)
      expect(handler).toHaveBeenCalledWith({ photoId: '1', projectId: 'project-1' })
    })
