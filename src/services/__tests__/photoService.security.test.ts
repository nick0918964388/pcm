...
describe('PhotoService - Security Features', () => {
  ...
  describe('validateFileSecurely', () => {
    it('should fail validation when rate limit is exceeded', async () => {
      ...
      // Act
      const result = await photoService.validateFileSecurely(file, mockUserId, mockProjectId)

      // Assert
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('上傳頻率過高，請稍後再試')
      expect(rateLimitService.checkUserRateLimit).toHaveBeenCalledWith(mockUserId, 'upload')
    })
    ...
    it('should pass validation when rate limit is not exceeded', async () => {
      ...
      expect(fileSecurityService.validateFileSecurely).toHaveBeenCalledWith(file, mockUserId, mockProjectId)
    })

    it('should handle security validation failures', async () => {
      // Arrange
      const file = new File(['malicious'], 'malware.exe', { type: 'application/exe' })
  ...
  describe('createUploadFile - Security Integration', () => {
    it('should create upload file with security validation', async () => {
      ...
      vi.mocked(fileSecurityService.generateSecureFilePath).mockReturnValue('/secure/path/secure_test.jpg')

      // Act
      const result = await photoService.createUploadFile(file, mockUserId, mockProjectId)
    ...
    it('should throw error when security validation fails', async () => {
      // Arrange
      const file = new File(['malicious'], 'malware.jpg', { type: 'image/jpeg' })
      ...
      // Act & Assert
      await expect(photoService.createUploadFile(file, mockUserId, mockProjectId))
  ...
  describe('downloadPhoto - Security', () => {
    it('should check rate limit before download', async () => {
      ...
      // Act & Assert
      await expect(photoService.downloadPhoto(photo, mockUserId))
        .rejects.toThrow('下載頻率過高，請稍後再試')

      expect(rateLimitService.checkUserRateLimit).toHaveBeenCalledWith(mockUserId, 'download')
    })
    ...
    it('should generate signed URL for secure download', async () => {
      ...
      expect(mockLink.href).toBe('https://secure.example.com/download?token=abc123')
  ...
  describe('downloadPhotos - Batch Security', () => {
    it('should check batch rate limit', async () => {
      ...
      // Act & Assert
      await expect(photoService.downloadPhotos(photos, mockUserId))
        .rejects.toThrow('批次操作頻率過高，請稍後再試')

      expect(rateLimitService.checkUserRateLimit).toHaveBeenCalledWith(mockUserId, 'batch')
    })
    ...
    it('should generate batch download signed URL', async () => {
      ...
      expect(signedUrlService.recordDownload).toHaveBeenCalledWith('batch-token-123')
    })
  })
  ...
  describe('Security Error Handling', () => {
    it('should handle security service errors gracefully', async () => {
      ...
      // Act & Assert
      await expect(photoService.validateFileSecurely(file, mockUserId, mockProjectId))
