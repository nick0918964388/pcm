...
import { useLoadingState, useMultipleLoadingState } from '@/hooks/useLoadingState'
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Button 
                  size="sm"
                  onClick={() => dashboardLoading.withLoading(() => simulateDataLoad())}
                  disabled={dashboardLoading.isLoading}
                >
                  {dashboardLoading.isLoading && <RefreshCw className="h-4 w-4 animate-spin mr-2" />}
                  載入 Dashboard
                </Button>
              </CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Button 
                  size="sm"
                  onClick={() => projectLoading.withLoading(() => simulateDataLoad(1500))}
                  disabled={projectLoading.isLoading}
                >
                  {projectLoading.isLoading && <RefreshCw className="h-4 w-4 animate-spin mr-2" />}
                  載入專案
                </Button>
              </CardTitle>
            </CardHeader>
      ...
      <section className="space-y-4">
              ...
              <CardHeader>
                <CardTitle className="flex items-center justify-between text-base">
                  ...
                  <Button 
                    size="sm"
                    variant="outline"
                    onClick={() => multipleLoading.withLoading(
                      `item-${index}`, 
        ...
        <div className="text-center">
          <p className="text-sm text-gray-600 mb-2">
            {multipleLoading.isAnyLoading() ? '有資料正在載入中...' : '所有資料載入完成'}
          </p>
          <Button
            onClick={() => {
              [0, 1, 2].forEach(index => 
                multipleLoading.withLoading(
                  `item-${index}`, 
  ...
  return (
    <Card>
      <CardContent className="p-6">
        <p className="text-green-600">組件載入成功！3秒後會拋出錯誤來示範錯誤處理。</p>
      </CardContent>
    </Card>
  )
}