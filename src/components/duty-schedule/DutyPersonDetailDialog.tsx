        ...
        <DialogHeader className='flex flex-row items-center justify-between space-y-0 pb-4'>
          <DialogTitle className='text-xl font-semibold flex items-center gap-3'>
            <span className='text-2xl'>
              {SHIFT_TYPE_ICONS[schedule.shiftType]}
            </span>
            <div>
              <div className='flex items-center gap-3'>
                ...
                <Badge
                  style={{
                    backgroundColor: DUTY_STATUS_COLORS[schedule.status],
                  }}
              ...
              <div className='text-sm text-gray-500 font-normal'>
                {formatDate(schedule.dutyDate)} • {schedule.shiftType} •{' '}
                {formatShiftTime(schedule.shiftTime)}
              </div>
            </div>
          </DialogTitle>
              ...
              <div className='grid grid-cols-1 gap-3'>
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    ...
                    <div className='font-medium'>
                      {formatDate(schedule.dutyDate)}
                    </div>
                    <div className='text-sm text-gray-600'>
                      {format(new Date(schedule.dutyDate), 'EEEE', {
                        locale: zhTW,
                      })}
                    </div>
                  </div>
                ...
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    ...
                    <div className='text-sm text-gray-600'>
                      {formatShiftTime(schedule.shiftTime)} (
                      {schedule.shiftTime.totalHours} 小時)
                    </div>
                  </div>
                </div>
                ...
                {schedule.urgencyLevel && (
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      ...
                      <Badge
                        variant={
                          schedule.urgencyLevel === UrgencyLevel.CRITICAL
                            ? 'destructive'
                            : 'secondary'
                        }
                        className='text-sm'
                    ...
                    <div>
                      ...
                      <div className='space-y-1'>
                        <div className='text-sm'>
                          <span className='font-medium'>原班人員:</span>{' '}
                          {schedule.replacement.originalPersonName}
                        </div>
                        <div className='text-sm'>
                          <span className='font-medium'>代班人員:</span>{' '}
                          {schedule.replacement.replacementPersonName}
                        </div>
                        <div className='text-sm'>
                          <span className='font-medium'>代班原因:</span>{' '}
                          {schedule.replacement.reason}
                        </div>
                    ...
                    <div>
                      ...
                      <div className='space-y-1'>
                        {schedule.checkIn.checkInTime && (
                          <div className='text-sm'>
                            <span className='font-medium'>簽到時間:</span>{' '}
                            {formatDateTime(schedule.checkIn.checkInTime)}
                          </div>
                        )}
                        {schedule.checkIn.checkOutTime && (
                          <div className='text-sm'>
                            <span className='font-medium'>簽退時間:</span>{' '}
                            {formatDateTime(schedule.checkIn.checkOutTime)}
                          </div>
                        )}
                        {schedule.checkIn.actualHours && (
                          <div className='text-sm'>
                            <span className='font-medium'>實際工時:</span>{' '}
                            {schedule.checkIn.actualHours} 小時
                          </div>
                        )}
              ...
              <div className='space-y-3'>
                ...
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    ...
                    <div className='font-medium'>
                      {schedule.person.vendorName}
                    </div>
                    <div className='text-sm text-gray-600'>
                      {schedule.person.vendorType}
                    </div>
                  </div>
                </div>
              ...
              {schedule.requirements && (
                <>
                  ...
                  <div className='space-y-3'>
                    ...
                    {schedule.requirements.specialSkills &&
                      schedule.requirements.specialSkills.length > 0 && (
                        <div>
                          <div className='text-sm text-gray-500 mb-1'>
                            特殊技能
                          </div>
                          <div className='flex flex-wrap gap-1'>
                            {schedule.requirements.specialSkills.map(
                              (skill, index) => (
                                <Badge
                                  key={index}
                                  variant='outline'
                    ...
                    {schedule.requirements.safetyQualifications &&
                      schedule.requirements.safetyQualifications.length > 0 && (
                        <div>
                          <div className='text-sm text-gray-500 mb-1'>
                            安全資格
                          </div>
                          <div className='flex flex-wrap gap-1'>
                            {schedule.requirements.safetyQualifications.map(
                              (qual, index) => (
                                <Badge
                                  key={index}
                                  variant='outline'
                    ...
                    {schedule.requirements.languageRequirements &&
                      schedule.requirements.languageRequirements.length > 0 && (
                        <div>
                          <div className='text-sm text-gray-500 mb-1'>
                            語言要求
                          </div>
                          <div className='flex flex-wrap gap-1'>
                            {schedule.requirements.languageRequirements.map(
                              (lang, index) => (
                                <Badge
                                  key={index}
                                  variant='outline'
