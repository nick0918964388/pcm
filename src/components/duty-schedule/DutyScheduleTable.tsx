...
import { Eye, Edit, Phone, Mail, Clock, MapPin, AlertTriangle } from 'lucide-react'
  ...
  const columns: Column<DutySchedule>[] = useMemo(() => [
    {
      ...
      render: (_, __, index) => {
        ...
        return <span className="font-mono text-xs">{baseIndex + index + 1}</span>
      }
    },
    {
      key: 'dutyDate',
      ...
      render: (value: Date) => (
        <div className="text-sm">
          <div className="font-medium">{format(new Date(value), 'MM/dd', { locale: zhTW })}</div>
          <div className="text-xs text-gray-500">{format(new Date(value), 'E', { locale: zhTW })}</div>
        </div>
      )
    },
    {
      ...
      render: (value: ShiftType, record: DutySchedule) => (
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-gray-500">{formatShiftTime(record.shiftTime)}</span>
          </div>
        </div>
      )
    },
    {
      ...
      render: (_, record: DutySchedule) => (
        <div className="space-y-1">
          <div className="font-medium text-sm">{record.person.vendorName}</div>
          <div className="text-xs text-gray-500">{record.person.vendorType}</div>
        </div>
      )
    },
    {
      ...
      render: (_, record: DutySchedule) => (
        <div className="space-y-1">
          ...
          <div className="text-xs text-gray-600">{record.person.position}</div>
          {record.person.isPrimary && (
            <Badge variant="secondary" className="text-xs">主聯絡人</Badge>
          )}
        </div>
      )
    },
    ...
    },
    {
      ...
      render: (_, record: DutySchedule) => {
        ...
        return <span className="font-mono text-sm">{record.person.extension}</span>
      }
    },
    {
      key: 'person.mvpn',
      ...
      render: (_, record: DutySchedule) => {
        ...
        return <span className="font-mono text-sm">{record.person.mvpn}</span>
      }
    },
    {
      key: 'person.email',
    ...
    },
    {
      ...
      render: (value: WorkArea, record: DutySchedule) => (
        <div className="space-y-1">
          ...
          {record.workLocation && (
            <div className="text-xs text-gray-500 truncate" title={record.workLocation}>
              {record.workLocation}
            </div>
          )}
        </div>
