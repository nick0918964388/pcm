...
import { Download, Share2, Info, X, ZoomIn, ZoomOut, Maximize, Grid3X3 } from 'lucide-react'
...
export function PhotoLightbox({
    ...
    indicesToPreload.forEach(idx => {
      ...
      if (photo && !preloadedImages.current.has(photo.originalUrl)) {
        const promise = new Promise<void>((resolve, reject) => {
          ...
          img.onerror = () => reject(new Error(`Failed to preload ${photo.fileName}`))
  ...
  }, [currentPhoto, handleDownload, handleShare, onClose, enableZoom, enableFullscreen, enableThumbnails])
        ...
        <div className="bg-black/80 backdrop-blur-sm rounded-lg p-4 text-white">
          ...
          {enableKeyboardShortcuts && (
            <>
              ...
              <div>
                <span className="text-gray-400 text-sm block mb-2">鍵盤快捷鍵</span>
  ...
  }, [currentPhoto, formatDate, formatFileSize, enableKeyboardShortcuts, index, photos.length])
  ...
  }, [open, enableKeyboardShortcuts, index, photos.length, handleIndexChange, handleDownload, handleShare])
