...
import { ImageIcon, Wifi, WifiOff, Settings, Zap, Clock } from 'lucide-react';
    ...
    <Card className='p-6'>
      ...
      <div className='grid grid-cols-2 gap-4 text-sm'>
        ...
        <div className='col-span-2 text-xs text-gray-500'>
          WebP支援: {metrics.webpSupported ? '是' : '否'} | 預載入:{' '}
          {strategy.preloadCount} 張 | 並發: {strategy.concurrency}
        </div>
      </div>
    </Card>
  );
...
export function OptimizedPhotoGallery({
  ...
  const { preload, progress, isPreloading } =
    useBatchProgressivePreload(thumbnailUrls);
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center space-x-4'>
          ...
          <Badge variant='outline'>{photos.length} 張照片</Badge>
          {isPreloading && (
