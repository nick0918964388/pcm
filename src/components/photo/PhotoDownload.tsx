...
const PhotoDownload: React.FC<PhotoDownloadProps> = ({
  ...
  // 處理下載
  const handleDownload = useCallback(async (resolution: PhotoResolution) => {
    try {
      // 生成下載ID
      ...
      const hasPermission = await service.validateDownloadPermissions(photo.id, userId)
      ...
      // 開始下載追蹤
      const fileName = service.generateDownloadFileName(photo.fileName, resolution)
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        {(['thumbnail', 'small', 'medium', 'large', 'original'] as PhotoResolution[]).map((resolution) => (
          <DropdownMenuItem
            key={resolution}
