...
export function PhotoFilters({
  ...
  // 更新單個篩選器
  const updateFilter = useCallback((key: keyof PhotoFiltersType, value: any) => {
    onFiltersChange({
      ...filters,
      [key]: value
    })
  ...
  // 移除特定篩選器
  const removeFilter = useCallback((key: keyof PhotoFiltersType) => {
      ...
      {activeFiltersCount > 0 && (
        <div className="flex items-center space-x-1 flex-wrap">
          ...
          {filters.dateRange && (
            <Badge variant="secondary" className="flex items-center space-x-1">
              <span className="text-xs">
                日期: {formatDateForInput(filters.dateRange.start)} ~ {formatDateForInput(filters.dateRange.end)}
              </span>
              <Button
                variant="ghost"
                size="sm"
