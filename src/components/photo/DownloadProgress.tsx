...
export interface BatchDownloadProgress {
  ...
  status: 'preparing' | 'downloading' | 'zipping' | 'completed' | 'failed' | 'cancelled'
...
export function DownloadProgress({
  ...
  const elapsedTime = Math.floor((currentTime - progress.startTime.getTime()) / 1000)
  const canCancel = progress.status === 'preparing' || progress.status === 'downloading' || progress.status === 'zipping'
          ...
          <div className="flex items-center justify-between mt-2 text-xs text-gray-600">
            <div className="flex items-center space-x-4">
              ...
              {progress.failedFiles > 0 && (
                <span className="text-red-600">失敗: {progress.failedFiles}</span>
              )}
            </div>
            <div className="flex items-center space-x-4">
              <span>已耗時: {formatTimeRemaining(elapsedTime)}</span>
              {progress.estimatedTimeRemaining && (
                <span>剩餘: {formatTimeRemaining(progress.estimatedTimeRemaining)}</span>
              )}
            </div>
          </div>
        </div>
