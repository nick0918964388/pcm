...
export function PhotoUploader({
  ...
  const handleUpload = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      // 開始上傳
      const results = await photoService.uploadPhotos(uploadFiles, (fileId, progress) => {
      ...
      if (failedResults.length > 0) {
        const errors = failedResults.flatMap(r => r.errors || ['Unknown error'])
  ...
  }, [projectId, albumId, disabled, addToUploadQueue, updateUploadProgress, updateUploadStatus, onUploadComplete, onUploadError, onUploadProgress])
  ...
  }, [isDragActive, isDragAccept, isDragReject, disabled, isUploading, className])
  ...
  return (
    <div className="w-full space-y-4">
      ...
      {uploadQueue.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">上傳佇列 ({uploadQueue.length})</CardTitle>
          </CardHeader>
          <CardContent>
