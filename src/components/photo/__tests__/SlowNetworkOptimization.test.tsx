...
describe('SlowNetworkOptimization', () => {
    ...
    test('should load low-quality images first on slow networks', async () => {
      ...
      images.forEach(img => {
        expect(img).toHaveAttribute(
          'src',
          expect.stringContaining('quality=30')
        );
      });
  ...
  describe('Upload Optimization', () => {
    test('should compress images more aggressively on slow networks', async () => {
      ...
      await waitFor(() => {
        const compressionIndicator = screen.getByTestId(
          'compression-indicator'
        );
        expect(compressionIndicator).toHaveTextContent(
          '高壓縮模式 (適用於慢速網路)'
        );
      });
    });
  ...
  describe('Lightbox Optimization', () => {
    test('should load medium quality images in lightbox on slow networks', async () => {
      ...
      expect(image).toHaveAttribute(
        'src',
        expect.stringContaining('quality=60')
      );
    ...
    test('should preload adjacent images with lower priority', async () => {
      ...
      await waitFor(() => {
        const preloadedImages = screen.getAllByTestId(
          'preloaded-lightbox-image'
        );
  ...
  describe('Data Usage Monitoring', () => {
    ...
    test('should offer reduced quality mode', async () => {
      ...
      images.forEach(img => {
        expect(img).toHaveAttribute(
          'src',
          expect.stringContaining('quality=20')
        );
      });
    });
  });
  ...
  describe('Background Sync', () => {
    test('should defer non-critical updates on slow networks', async () => {
      ...
      await waitFor(() => {
        const backgroundSyncIndicator = screen.getByTestId(
          'background-sync-indicator'
        );
        expect(backgroundSyncIndicator).toHaveTextContent(
          '背景同步已暫停以節省流量'
        );
      });
    });
  ...
  describe('Performance Metrics', () => {
    test('should measure and display load times', async () => {
      ...
      await waitFor(
        () => {
          const performanceIndicator = screen.getByTestId(
    ...
    test('should adapt behavior based on performance metrics', async () => {
      ...
      await waitFor(() => {
        const adaptiveIndicator = screen.getByTestId(
          'adaptive-loading-indicator'
        );
        expect(adaptiveIndicator).toHaveTextContent(
          '已調整載入策略以適應網路狀況'
        );
      });
    });
