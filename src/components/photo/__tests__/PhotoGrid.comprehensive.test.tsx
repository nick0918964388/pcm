...
describe('PhotoGrid Component', () => {
  ...
  describe('Selection Functionality', () => {
    ...
    it('should show selected photos count', () => {
      render(
        <PhotoGrid {...defaultProps} selectedPhotos={['photo-1', 'photo-2']} />
      );
  ...
  describe('Photo Interactions', () => {
    ...
    it('should handle photo download from dropdown menu', async () => {
      ...
      render(
        <PhotoGrid {...defaultProps} onPhotoDownload={mockOnPhotoDownload} />
      );

      // Hover over first photo to show dropdown
      const firstPhotoCard = screen
        .getAllByTestId('photo-thumbnail')[0]
        .closest('.group');
      ...
      const moreButton = screen
        .getAllByRole('button')
        .find(btn => btn.querySelector('[data-testid="more-horizontal"]'));
      if (moreButton) {
    ...
    it('should handle batch download', async () => {
      ...
      expect(mockOnPhotoDownload).toHaveBeenCalledWith([
        mockPhotos[0],
        mockPhotos[1],
      ]);
    });
  ...
  describe('Storage Management', () => {
    it('should show storage usage indicator when enabled', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId('storage-usage-indicator')
        ).toBeInTheDocument();
      });
    });
    ...
    it('should show storage warning when space is low', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('儲存空間不足，建議清理快取')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Virtualization Features', () => {
    ...
    it('should not show virtualization for small photo sets', () => {
      render(<PhotoGrid {...defaultProps} enableVirtualization={true} />);

      expect(
        screen.queryByTestId('virtual-grid-container')
