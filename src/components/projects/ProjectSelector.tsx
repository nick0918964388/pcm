 ...
 * - 專案選擇
 */
export function ProjectSelector({ inNavbar = false, className = '' }: ProjectSelectorProps) {
  const router = useRouter()
  const dropdownRef = useRef<HTMLDivElement>(null)
  
  const currentProject = useCurrentProject()
  const recentProjects = useRecentProjects(8)
  const { favoriteProjectIds, toggleFavorite, isFavorite } = useFavoriteProjects()
  const {
  ...
  const { projects, searchProjects, filteredProjects, initialized, initialize } = useProjectStore()
  ...
  const getDisplayProjects = () => {
    switch (activeTab) {
      case 'recent':
        return recentProjects.map(record => 
      ...
      <Card className="p-6">
        ...
        {currentProject ? (
          ...
          <div className="text-center py-8 bg-gray-50 rounded-lg mb-4">
            ...
            <Button onClick={handleGoToProjectSelection}>
              選擇專案
            </Button>
          </div>
        )}
