...
export function ProjectMemberSearch({
  ...
  // 使用搜索 hook
  const { suggestions, isLoading, search, clearSearch } =
    useProjectMemberSearch(projectId, { debounceMs: 300, minLength: 2 });
  ...
  const handleSuggestionSelect = useCallback(
    (suggestion: SearchSuggestion) => {
      ...
      setSearchHistory(prev => {
        const newHistory = [
          suggestion.value,
          ...prev.filter(item => item !== suggestion.value),
        ];
