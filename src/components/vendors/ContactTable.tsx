...
export function ContactTable({
  ...
  const columns: Column<VendorContact>[] = useMemo(() => [
        ...
        <div className="flex items-center gap-2">
          <div>
            <div className="font-medium text-sm flex items-center gap-1">
              {value}
              {record.isPrimary && (
                <Star className="h-3 w-3 text-yellow-500 fill-current" title="主要聯絡人" />
              )}
            </div>
            {record.position && (
    ...
    },
    {
      ...
      sortable: true,
      render: (value: string) => (
        value ? <span className="text-sm">{value}</span> : <span className="text-gray-400 text-sm">-</span>
      )
    },
    {
      key: 'phone',
    ...
    },
    {
      ...
      width: '80px',
      render: (value: string) => (
        value ? <span className="font-mono text-sm">{value}</span> : <span className="text-gray-400 text-sm">-</span>
      )
    },
    {
      key: 'mvpn',
    ...
    },
    {
      ...
      render: (value: boolean) => (
        <div className="flex items-center justify-center">
          {value ? (
            <Star className="h-4 w-4 text-yellow-500 fill-current" title="主要聯絡人" />
          ) : (
