...
export function ContactCard({ contact, onEdit, onCall, onEmail }: ContactCardProps) {
  return (
      ...
      <CardContent className="p-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#1A1A1A] flex-1">{contact.phone}</span>
            {onCall && (
              <Button 
                size="sm" 
                variant="ghost" 
          ...
          {contact.mobile && (
            <div className="flex items-center gap-2">
              <Phone className="h-4 w-4 text-[#8C8C8C]" />
              <span className="text-sm text-[#1A1A1A] flex-1">{contact.mobile}</span>
              {onCall && (
                <Button 
                  size="sm" 
                  variant="ghost" 
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#1A1A1A] flex-1 truncate">{contact.email}</span>
            {onEmail && (
              <Button 
                size="sm" 
                variant="ghost" 
          ...
          {contact.extension && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-[#1A1A1A]">{contact.extension}</span>
            </div>
          )}
        </div>
        ...
        {contact.notes && (
          <div className="mt-3 pt-3 border-t border-[#F0F0F0]">
            <p className="text-sm text-[#595959] overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
              {contact.notes}
            </p>
          </div>
        )}
