...
export function MilestoneTimeline({
  ...
  return (
    <div
      className={cn(
        'bg-white p-4 sm:p-6 rounded shadow-[0_2px_8px_rgba(0,0,0,0.06)]',
        className
      )}
    >
      <h3 className='text-base sm:text-lg font-bold text-[#1A1A1A] mb-6 sm:mb-8'>
        Milestone
      </h3>
          ...
          {milestones.map((milestone, index) => (
            <div key={index} className='flex flex-col items-center'>
              ...
              <div className='mt-3 sm:mt-4 text-center max-w-16 sm:max-w-none'>
                <p className='text-xs sm:text-xs text-[#8C8C8C] leading-tight'>
                  {milestone.date}
                </p>
