...
export default function ProjectStaffPage() {
  ...
  const [filters, setFilters] = useState<ProjectMemberSearchParams>(initialFilters)
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    專案 ID: {projectId} | 
                    顯示 {paginatedMembers.length} / {filteredMembers.length} 位成員
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-2xl font-bold text-gray-900">
                      {Math.round(mockMembers.reduce((sum, m) => sum + m.workload, 0) / mockMembers.length)}%
                    </p>
            ...
            <Card>
              <CardContent className="flex items-center justify-between py-4">
                <div className="flex items-center space-x-2 text-sm text-gray-600">
                  <span>
                    顯示第 {((filters.page || 1) - 1) * (filters.pageSize || 12) + 1} - {Math.min((filters.page || 1) * (filters.pageSize || 12), filteredMembers.length)} 項，
                    共 {filteredMembers.length} 項結果
                  </span>
                </div>
                <div className="flex items-center space-x-2">
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      const updatedFilters = { ...filters, page: Math.max(1, (filters.page || 1) - 1) }
                      setFilters(updatedFilters)
                      updateUrlParams(updatedFilters)
                    }}
                  ...
                  <div className="flex items-center space-x-1">
                    {(() => {
                      const totalPages = Math.ceil(filteredMembers.length / (filters.pageSize || 12))
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const totalPages = Math.ceil(filteredMembers.length / (filters.pageSize || 12))
                      const updatedFilters = { ...filters, page: Math.min(totalPages, (filters.page || 1) + 1) }
                      setFilters(updatedFilters)
                      updateUrlParams(updatedFilters)
                    }}
                    disabled={(filters.page || 1) >= Math.ceil(filteredMembers.length / (filters.pageSize || 12))}
                  >
                    下一頁
                  </Button>
                </div>
            ...
            <Card>
              <CardContent className="flex items-center justify-center py-12">
                <div className="text-center text-gray-500">
                  ...
                  <p>沒有找到符合條件的專案成員</p>
                  {(searchQuery || filters.role || filters.department || filters.workStatus) && (
