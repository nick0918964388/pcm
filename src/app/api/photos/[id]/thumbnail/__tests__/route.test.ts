...
describe('照片縮圖API測試', () => {
  ...
  describe('GET /api/photos/[id]/thumbnail', () => {
    it('應該回傳照片縮圖', async () => {
      ...
      mockConnection.query.mockResolvedValueOnce({ rows: [mockPhoto] }); // 查詢照片

      const request = new NextRequest(
        `http://localhost/api/photos/${photoId}/thumbnail`
      );
    ...
    it('應該支援不同尺寸的縮圖', async () => {
      ...
      mockConnection.query.mockResolvedValueOnce({ rows: [mockPhoto] });

      const request = new NextRequest(
        `http://localhost/api/photos/${photoId}/thumbnail?size=large`
      );
