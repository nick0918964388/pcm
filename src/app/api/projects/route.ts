...
const ProjectSchema = z.object({
  ...
  status: z
    ...
    .default('planning'),
  type: z
    .enum(['construction', 'infrastructure', 'maintenance'])
...
async function getProjectsFromOracle(request: NextRequest) {
  try {
    // 簡單查詢，不使用複雜參數
    const query =
      'SELECT id, name, description, status, created_at, updated_at, is_active FROM projects WHERE is_active = 1 ORDER BY updated_at DESC';
...
async function createProjectInOracle(request: NextRequest) {
  try {
    ...
    // 查詢新建立的專案
    const result = await db.queryOne('SELECT * FROM projects WHERE id = :1', [
      body.id,
    ]);
