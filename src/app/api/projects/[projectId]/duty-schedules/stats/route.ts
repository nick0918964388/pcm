...
export async function GET(
  ...
  try {
    ...
    // 取得排程統計
    const stats = await dutyScheduleService.getScheduleStatistics(
      projectId,
      dateFrom,
      dateTo
    );
    ...
    const replacementNeeded =
      await dutyScheduleService.getReplacementNeeded(projectId);

    // 取得緊急排程
    const urgentSchedules =
      await dutyScheduleService.getUrgentSchedules(projectId);
