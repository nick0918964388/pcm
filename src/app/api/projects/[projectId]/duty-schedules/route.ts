...
const CreateScheduleSchema = z.object({
  ...
  dutyDate: z.string().datetime().transform(date => new Date(date)),
...
export async function GET(
  ...
  try {
    ...
    const { page, limit, dateFrom, dateTo, status, shiftType, urgencyLevel, personId } = validatedQuery;
...
export async function POST(
  ...
  try {
    ...
    // 建立排程
    const schedule = await dutyScheduleService.createSchedule(scheduleData, createdBy);
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('已有其他值班安排') || error.message.includes('不能為過去的日期')) {
        return NextResponse.json(
          { error: error.message },
          { status: 409 }
        );
