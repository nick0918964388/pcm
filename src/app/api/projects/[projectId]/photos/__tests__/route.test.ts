  ...
  describe('GET /api/projects/[projectId]/photos', () => {
    it('應該回傳專案照片列表', async () => {
      ...
      const request = new NextRequest(
        `http://localhost/api/projects/${projectId}/photos`
      );
    ...
    it('應該支援分頁查詢', async () => {
      ...
      const request = new NextRequest(
        `http://localhost/api/projects/${projectId}/photos?page=1&limit=10`
      );
    ...
    it('應該回傳錯誤當專案不存在時', async () => {
      ...
      mockConnection.query.mockResolvedValueOnce({ rows: [] }); // 專案不存在
  ...
  describe('POST /api/projects/[projectId]/photos', () => {
    it('應該驗證檔案格式', async () => {
      ...
      mockConnection.query.mockResolvedValueOnce({
        rows: [{ id: 'album1', project_id: projectId, name: 'Test Album' }],
      });
