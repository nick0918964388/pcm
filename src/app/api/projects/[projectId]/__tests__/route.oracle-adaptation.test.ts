...
vi.mock('@/lib/repositories/oracle-project-repository', () => ({
  OracleProjectRepository: vi.fn().mockImplementation(() => mockOracleProjectRepository)
}))
...
describe('Projects API Oracle適配測試', () => {
  ...
  describe('RED: GET /api/projects/[projectId] - Oracle查詢語法適配', () => {
    it('應該使用Oracle特定的VARCHAR2 UUID查詢語法', async () => {
      ...
      mockOracleProjectRepository.findByProjectCode.mockResolvedValue(mockProject)
      ...
      expect(mockOracleProjectRepository.findByProjectCode).toHaveBeenCalledWith(projectId)
    ...
    it('應該處理Oracle錯誤並返回適當的HTTP狀態碼', async () => {
      ...
      mockOracleProjectRepository.findByProjectCode.mockRejectedValue(oracleError)
  ...
  describe('RED: PUT /api/projects/[projectId] - Oracle更新語法適配', () => {
    ...
    it('應該處理Oracle約束違反錯誤', async () => {
      ...
      const oracleConstraintError = new Error('ORA-01400: cannot insert NULL into column')
      mockOracleProjectRepository.findByProjectCode.mockResolvedValue({
        id: projectId,
        name: '現有專案'
      })
      mockOracleProjectRepository.update.mockRejectedValue(oracleConstraintError)
  ...
  describe('RED: DELETE /api/projects/[projectId] - Oracle軟刪除語法適配', () => {
    it('應該使用Oracle軟刪除語法（UPDATE ... SET deleted_at = SYSDATE）', async () => {
      ...
      expect(response.status).toBe(200)
      expect(mockOracleProjectRepository.findByProjectCode).toHaveBeenCalledWith(projectId)
      expect(mockOracleProjectRepository.softDelete).toHaveBeenCalledWith(projectId)
      expect(data.success).toBe(true)
