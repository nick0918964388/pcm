...
async function checkConnectionPoolHealth(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    return {
      service: 'connection-pool',
      status: 'unhealthy',
      details: error instanceof Error ? error.message : 'Pool status check failed',
      responseTime,
      timestamp: new Date().toISOString()
    }
  }
...
function determineOverallHealth(services: HealthCheckResult[]): 'healthy' | 'unhealthy' | 'degraded' {
  ...
  try {
    ...
    const errorResponse: SystemHealth = {
      ...
      services: [{
        ...
        status: 'unhealthy',
        details: error instanceof Error ? error.message : 'Health check system error',
        timestamp: new Date().toISOString()
      }],
      timestamp: new Date().toISOString(),
