...
describe('ProjectCard', () => {
  ...
  describe('基本渲染', () => {
    it('應該正確渲染專案卡片的基本結構', () => {
      ...
      expect(screen.getByLabelText(`進入專案 ${mockProject.name}`)).toBeInTheDocument()
    })
    ...
    it('應該在載入中時套用正確的樣式和覆蓋層', () => {
      ...
      // 檢查載入中覆蓋層
      const loadingOverlay = document.querySelector('.absolute.inset-0.bg-background\\/50')
  ...
  describe('專案類型顯示', () => {
    ...
    it('應該為建築工程項目顯示正確的圖標', () => {
      const constructionProject = mockProjects.find(p => p.type === ProjectType.CONSTRUCTION)
    ...
    it('應該為翻新工程項目顯示正確的圖標', () => {
      const renovationProject = mockProjects.find(p => p.type === ProjectType.RENOVATION)
    ...
    it('應該為維護工程項目顯示正確的圖標', () => {
      const maintenanceProject = mockProjects.find(p => p.type === ProjectType.MAINTENANCE)
  ...
  describe('時間資訊顯示', () => {
    ...
    it('應該為即將到期的專案顯示警告樣式', () => {
      ...
      expect(remainingDaysText).toHaveClass('text-orange-600', 'dark:text-orange-400')
    })
  })
  ...
  describe('專案點擊導航功能 (US5 AC5.1)', () => {
    ...
    it('應該在進入專案失敗時恢復按鈕狀態', async () => {
      ...
      const failingOnProjectEnter = vi.fn().mockRejectedValue(new Error('Network error'))
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error entering project:', expect.any(Error))
  ...
  describe('可訪問性', () => {
    it('應該提供正確的 ARIA 標籤', () => {
      ...
      const card = screen.getByRole('button')
      expect(card).toHaveAttribute('aria-label', `進入專案 ${mockProject.name}`)
    })
    ...
    it('應該為進度條提供正確的 ARIA 屬性', () => {
      ...
      const progressBar = screen.getByLabelText(`專案進度 ${Math.round(mockProject.progress)}%`)
      expect(progressBar).toBeInTheDocument()
    })
  ...
  describe('邊界條件', () => {
    ...
    it('應該處理非常長的專案經理姓名', () => {
      ...
      const managerElement = screen.getByText(projectWithLongManagerName.managerName)
