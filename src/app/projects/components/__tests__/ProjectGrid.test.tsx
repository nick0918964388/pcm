...
describe('ProjectGrid', () => {
  // 基本渲染測試
  it('應該正確渲染專案網格', () => {
    render(<ProjectGrid projects={mockProjects} loading={false} />);
  ...
  it('應該在沒有專案時顯示空狀態', () => {
    ...
    expect(
      screen.getByText('目前沒有符合條件的專案資料。請檢查篩選條件或稍後再試。')
    ).toBeInTheDocument();
  });
  ...
  it('應該在出現錯誤時顯示錯誤訊息', () => {
    ...
    render(<ProjectGrid projects={[]} loading={false} error={errorMessage} />);
  ...
  it('應該在空狀態下支援重新整理功能', () => {
    const onRetryMock = jest.fn();
    render(<ProjectGrid projects={[]} loading={false} onRetry={onRetryMock} />);
  ...
  it('應該支援緊湊模式', () => {
    render(
      <ProjectGrid projects={mockProjects} loading={false} compact={true} />
    );
  ...
  it('應該在有資料且載入中時顯示載入更多指示器', () => {
    render(<ProjectGrid projects={mockProjects} loading={true} />);
