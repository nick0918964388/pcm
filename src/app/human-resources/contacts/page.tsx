...
import { DataTable, Column, FilterBar, FilterConfig, Modal } from '@/components/shared'
...
export default function VendorContactsPage() {
  ...
  const filteredContacts = React.useMemo(() => {
    ...
    if (filterValues.type && filterValues.type !== 'all') {
      ...
      filtered = filtered.filter(contact => contact.type === typeMap[filterValues.type])
    }
    ...
    },
    {
      ...
      render: (value, record) => (
        <div>
          ...
          {record.department && (
            <div className="text-xs text-[#8C8C8C] mt-1">{record.department}</div>
          )}
        </div>
      )
    },
    ...
    },
    {
      ...
      render: (value) => (
        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${
          value === '結構廠商' ? 'bg-blue-100 text-blue-800' :
          value === '機電廠商' ? 'bg-purple-100 text-purple-800' :
    ...
    <div className="space-y-6">
      ...
      <DataTable
        ...
        pagination={{
          ...
          onChange: (page, pageSize) => console.log('分頁變更:', page, pageSize)
        }}
