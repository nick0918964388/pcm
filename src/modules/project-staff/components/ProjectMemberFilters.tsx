...
import { ProjectMemberFilters as FilterTypes, WorkStatus } from '@/types/project'
...
export const ProjectMemberFilters: React.FC<ProjectMemberFiltersProps> = ({
  ...
  const handleWorkStatusChange = useCallback((selectedStatuses: WorkStatus[]) => {
    onFiltersChange({
      ...filters,
  ...
  const handleDepartmentChange = useCallback((selectedDepartments: string[]) => {
    onFiltersChange({
      ...filters,
  ...
  const handleWorkloadRangeChange = useCallback((min?: number, max?: number) => {
  ...
  const handleJoinDateRangeChange = useCallback((start?: Date, end?: Date) => {
    if (start || end) {
      onFiltersChange({
        ...filters,
  ...
  const renderWorkStatusSelect = () => (
    <div className="space-y-2">
      ...
      <select
        ...
        onChange={(e) => {
          const values = Array.from(e.target.selectedOptions, option => option.value as WorkStatus)
      ...
      {filters.workStatus && filters.workStatus.length > 0 && (
        <input
          type="text"
          readOnly
          value={filters.workStatus.map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(', ')}
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <input
          ...
          type="date"
          value={filters.joinDateRange?.start?.toISOString().split('T')[0] || ''}
          onChange={(e) => {
    ...
    >
      ...
      {(!collapsible || isExpanded) && (
        <div className="p-4">
          <div className={cn(
            'grid gap-4',
            responsive ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-3'
          )}>
            {/* 角色篩選 */}
            {renderMultiSelect(
              'role-filter',
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) => {
                    handleActiveStatusChange(e.target.checked ? true : undefined)
                  }}
