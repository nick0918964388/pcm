...
export const WBSNode: React.FC<WBSNodeProps> = ({
  ...
  const handleDragEnter = useCallback(
    (e: React.DragEvent) => {
      e.preventDefault();
      if (!draggable || readonly || dragState?.draggedNodeId === node.id)
        return;
          ...
          <div className='flex items-center space-x-4 mt-2 text-xs text-gray-500 dark:text-gray-400'>
            ...
            {(node.startDate || node.endDate) && (
              <div className='flex items-center'>
                ...
                <span>
                  {node.startDate &&
                    new Date(node.startDate).toLocaleDateString('zh-TW')}
                  {node.startDate && node.endDate && ' - '}
                  {node.endDate &&
                    new Date(node.endDate).toLocaleDateString('zh-TW')}
                </span>
              </div>
            )}
        ...
        {!readonly && (isHovered || selected) && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align='end'>
              <DropdownMenuItem onClick={handleEdit}>編輯節點</DropdownMenuItem>
