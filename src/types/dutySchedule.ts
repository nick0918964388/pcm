 ...
 * 值班查詢 API 回應
 */
export interface DutyScheduleListResponse
  extends ApiResponse<DutyScheduleQueryResult> {}
...
export function isShiftActive(
  ...
  const [startHour, startMinute] = schedule.shiftTime.startTime
    .split(':')
    .map(Number);
  const [endHour, endMinute] = schedule.shiftTime.endTime
    .split(':')
    .map(Number);
...
export function generateMockDutySchedules(count: number = 50): DutySchedule[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const personName =
      mockPersonNames[Math.floor(Math.random() * mockPersonNames.length)];
    const position =
    ...
    const workArea =
      workAreaValues[Math.floor(Math.random() * workAreaValues.length)];
    ...
    const person: DutyPerson = {
      ...
      mvpn:
        Math.random() > 0.7
          ? `mvpn-${String(Math.floor(Math.random() * 999) + 100)}`
          : undefined,
      vendorId: vendor.id,
    ...
    const schedule: DutySchedule = {
      ...
      workLocation:
        Math.random() > 0.5
          ? mockWorkLocations[
              Math.floor(Math.random() * mockWorkLocations.length)
            ]
          : undefined,
      replacement:
        Math.random() > 0.9
          ? {
              ...
              originalPersonName:
                mockPersonNames[
                  Math.floor(Math.random() * mockPersonNames.length)
                ],
      ...
      requirements:
        Math.random() > 0.7
          ? {
              specialSkills: ['高空作業', '電氣維修'].filter(
                () => Math.random() > 0.5
              ),
              safetyQualifications: ['工安證照', '急救證照'].filter(
                () => Math.random() > 0.5
              ),
              languageRequirements: ['中文', '英文'].filter(
                () => Math.random() > 0.7
              ),
            }
          : undefined,
      ...
      specialInstructions:
        Math.random() > 0.8 ? '需配合夜間施工作業' : undefined,
      urgencyLevel:
        Math.random() > 0.8
          ? Object.values(UrgencyLevel)[
              Math.floor(Math.random() * Object.values(UrgencyLevel).length)
            ]
          : undefined,
      checkIn:
        status === DutyStatus.ON_DUTY || status === DutyStatus.OFF_DUTY
          ? {
              checkInTime: new Date(
                dutyDate.getTime() + Math.random() * 3600000
              ),
              checkOutTime:
                status === DutyStatus.OFF_DUTY
                  ? new Date(
                      dutyDate.getTime() + 8 * 3600000 + Math.random() * 3600000
                    )
                  : undefined,
              actualHours:
                status === DutyStatus.OFF_DUTY
  ...
  return schedules.sort(
    (a, b) => new Date(b.dutyDate).getTime() - new Date(a.dutyDate).getTime()
  );
}
...
export function generateMockDutyScheduleQueryResult(
  ...
  if (filters.dateRange) {
    allSchedules = allSchedules.filter(schedule => {
      ...
      return (
        scheduleDate >= filters.dateRange!.from &&
        scheduleDate <= filters.dateRange!.to
      );
    });
  }
  ...
  if (filters.urgencyLevels?.length) {
    allSchedules = allSchedules.filter(
      schedule =>
        ...
        filters.urgencyLevels!.includes(schedule.urgencyLevel)
    );
  }
  ...
  const pageSize =
    pagination.pageSize || DUTY_SCHEDULE_CONSTANTS.DEFAULT_PAGE_SIZE;
  ...
  const stats: DutyScheduleStats = {
    ...
    activeSchedules: allSchedules.filter(s => s.status !== DutyStatus.CANCELLED)
      .length,
    currentOnDutyCount: allSchedules.filter(
      s => s.status === DutyStatus.ON_DUTY
    ).length,
    byShiftType: Object.values(ShiftType).reduce(
      (acc, type) => {
    ...
    byStatus: Object.values(DutyStatus).reduce(
      (acc, status) => {
    ...
    byVendor: allSchedules.reduce(
      ...
      {} as Record<
        string,
        { vendorName: string; count: number; currentOnDuty: number }
      >
    ),
    byWorkArea: Object.values(WorkArea).reduce(
      (acc, area) => {
    ...
    currentOnDuty: allSchedules
      .filter(s => s.status === DutyStatus.ON_DUTY)
      .slice(0, 10),
    upcomingDuties: allSchedules
      .filter(
        s =>
          s.status === DutyStatus.SCHEDULED && new Date(s.dutyDate) > new Date()
      )
      .slice(0, 5),
    emergencyContacts: allSchedules
      .filter(
        s =>
          s.shiftType === ShiftType.EMERGENCY ||
          s.urgencyLevel === UrgencyLevel.CRITICAL
      )
      .slice(0, 3),
    alertsCount: allSchedules.filter(s => s.status === DutyStatus.ABSENT)
      .length,
    anomaliesCount: allSchedules.filter(
      s => s.replacement || s.status === DutyStatus.CANCELLED
    ).length,
  };
