...
async function executeOracleQuery(query: string): Promise<any> {
  ...
  try {
    ...
    throw new Error(
      `Failed to execute Oracle query: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
describe('Task 9.1: Oracle Migration End-to-End Integration Tests', () => {
  beforeAll(async () => {
    ...
    try {
      ...
      throw new Error(
        `Oracle setup failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Oracle database setup failed: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  ...
  describe('1. 完整使用者工作流程測試', () => {
    it('應該能夠完成專案建立到照片管理的完整流程', async () => {
      ...
      const dbResult = await executeOracleQuery(
        `SELECT name, status FROM projects WHERE id = '${TEST_PROJECT.id}'`
      );
      ...
      const albumResponse = await makeApiRequest(
        `/api/projects/${TEST_PROJECT.id}/albums`,
        {
          method: 'POST',
      ...
      expect(
        projectsResponse.data.some((p: any) => p.id === TEST_PROJECT.id)
      ).toBe(true);
      ...
      // 步驟6: 驗證更新在Oracle中生效
      const updatedDbResult = await executeOracleQuery(
        `SELECT progress FROM projects WHERE id = '${TEST_PROJECT.id}'`
      );
    ...
    it('應該能夠處理JSON metadata欄位', async () => {
      ...
      // 驗證Oracle JSON查詢功能
      const dbResult = await executeOracleQuery(
        `SELECT JSON_VALUE(metadata, '$.location') as location FROM projects WHERE id = '${projectWithMetadata.id}'`
      );
  ...
  describe('2. 錯誤處理和恢復機制測試', () => {
    ...
    it('應該能夠從連線失敗中恢復', async () => {
      ...
      try {
        ...
        // 確保容器恢復運行
        await execAsync(
          `docker unpause ${TEST_CONFIG.ORACLE_CONTAINER} 2>/dev/null || true`
        );
      }
    });
  });
  ...
  describe('3. 系統穩定性測試', () => {
    it('應該能夠處理多個並發請求', async () => {
      ...
      // 驗證所有專案都在資料庫中
      const dbResult = await executeOracleQuery(
        `SELECT COUNT(*) as count FROM projects WHERE id LIKE 'TEST_CONCURRENT_%'`
      );
    ...
    it('應該維持長時間運行的穩定性', async () => {
      ...
      // 驗證資料完整性
      const dbResult = await executeOracleQuery(
        `SELECT COUNT(*) as count FROM projects WHERE id LIKE 'TEST_STABILITY_%'`
      );
  ...
  describe('4. API功能正確性驗證', () => {
    ...
    it('應該正確處理日期時間查詢', async () => {
      ...
      const response = await makeApiRequest(
        '/api/projects?start_date_from=2024-01-01&start_date_to=2024-02-01'
      );

      expect(response.data.some((p: any) => p.id === projectWithDates.id)).toBe(
        true
      );

      // 驗證Oracle日期格式
      const dbResult = await executeOracleQuery(
        `SELECT TO_CHAR(start_date, 'YYYY-MM-DD') as start_date FROM projects WHERE id = '${projectWithDates.id}'`
      );
