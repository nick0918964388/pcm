...
// 輔助函數
async function measureApiResponseTime(endpoint: string, method = 'GET', body?: any): Promise<number> {
  const startTime = Date.now()

  try {
    const response = await fetch(`${PERFORMANCE_CONFIG.API_BASE_URL}${endpoint}`, {
      method,
      headers: {
    ...
    throw new Error(`Performance test API request failed: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
describe('Task 9.2: Oracle Performance and Load Validation Tests', () => {
  ...
  describe('1. API回應時間驗證', () => {
    it('專案查詢API回應時間應符合150%基準要求', async () => {
      ...
      console.log(`Maximum acceptable: ${PERFORMANCE_CONFIG.BASELINE_RESPONSE_TIME * PERFORMANCE_CONFIG.MAX_ACCEPTABLE_RATIO}ms`)

      expect(responseTime).toBeLessThanOrEqual(
        PERFORMANCE_CONFIG.BASELINE_RESPONSE_TIME * PERFORMANCE_CONFIG.MAX_ACCEPTABLE_RATIO
      )
    })
    ...
    it('單個專案查詢回應時間應小於100ms', async () => {
      ...
      // 測試單個專案查詢
      const responseTime = await measureApiResponseTime(`/api/projects/PERF_TEST_001`)
    ...
    it('分頁查詢回應時間應小於200ms', async () => {
      ...
      // 測試分頁查詢
      const responseTime = await measureApiResponseTime('/api/projects?page=2&limit=10')
  ...
  describe('2. 並發處理能力測試', () => {
    it('應該能夠處理20個並發使用者查詢', async () => {
      ...
      // 驗證所有請求都成功
      expect(loadTestResult.successfulRequests).toBe(PERFORMANCE_CONFIG.CONCURRENT_USERS)
    })
    ...
    it('應該能夠處理混合讀寫操作', async () => {
      const requests = Array.from({ length: 50 }, (_, i) => {
        if (i % 5 === 0) {
          // 20% 寫入操作
          return () => measureApiResponseTime('/api/projects', 'POST', {
            id: `MIXED_TEST_${i}`,
            name: `混合測試專案 ${i}`,
            status: 'active',
            type: 'construction'
  ...
  describe('3. 資源使用效率測試', () => {
    it('資料庫連線池使用率應該合理', async () => {
      ...
      // 執行一些並發查詢
      await runConcurrentRequests(10, () => measureApiResponseTime('/api/projects'))
    ...
    it('系統資源使用應該在限制範圍內', async () => {
      ...
      // 同時執行負載測試
      const loadTestPromise = runConcurrentRequests(30, () => measureApiResponseTime('/api/projects'))

      const [metrics, loadResult] = await Promise.all([metricsPromise, loadTestPromise])
      ...
      // 驗證記憶體使用在限制內
      expect(metrics.memoryUsage).toBeLessThanOrEqual(PERFORMANCE_CONFIG.MEMORY_LIMIT_MB)

      // 驗證CPU使用率合理
      expect(metrics.cpuUsage).toBeLessThanOrEqual(PERFORMANCE_CONFIG.CPU_LIMIT_PERCENT)
  ...
  describe('4. 壓力測試', () => {
    it('應該能夠承受持續的高負載', async () => {
      console.log(`Starting stress test: ${PERFORMANCE_CONFIG.STRESS_TEST_REQUESTS} requests...`)
    ...
    it('系統應該從高負載後快速恢復', async () => {
      // 先執行高負載
      await runConcurrentRequests(50, () => measureApiResponseTime('/api/projects'))

      // 等待系統恢復
      await new Promise(resolve => setTimeout(resolve, 3000))

      // 測試恢復後的效能
      const recoveryResponseTime = await measureApiResponseTime('/api/projects')

      console.log(`Post-stress recovery response time: ${recoveryResponseTime}ms`)
      ...
      expect(poolStatus.activeConnections).toBeLessThanOrEqual(poolStatus.totalConnections)
    })
  })
  ...
  describe('5. 記憶體洩漏檢測', () => {
    it('長時間運行不應該有記憶體洩漏', async () => {
      ...
      // 執行長時間的重複操作
      for (let i = 0; i < 20; i++) {
        await runConcurrentRequests(5, () => measureApiResponseTime('/api/projects'))
