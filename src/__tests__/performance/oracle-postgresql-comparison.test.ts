...
describe('Oracle vs PostgreSQL Performance Comparison', () => {
  ...
  afterAll(async () => {
    ...
    if (failedTests.length > 0) {
      ...
      failedTests.forEach(test => {
        console.log(
          `  ${test.operation}: ${test.ratio}x (limit: ${BENCHMARK_CONFIG.MAX_ACCEPTABLE_RATIO}x)`
        );
      });
    } else {
  ...
  describe('基本查詢效能比較', () => {
    it('簡單查詢效能應符合150%限制', async () => {
      const result = await benchmarkOperation(
        'Simple Query',
        async () => {
          const queryResult = await oracle.executeOne(
      ...
      console.log(
        `Simple Query - Oracle: ${result.oracleTime}ms, PostgreSQL baseline: ${result.postgresqlTime}ms, Ratio: ${result.ratio}x`
      );

      expect(result.withinLimit).toBe(true);
      expect(result.ratio).toBeLessThanOrEqual(
        BENCHMARK_CONFIG.MAX_ACCEPTABLE_RATIO
      );
    });
    ...
    it('複雜查詢效能應符合150%限制', async () => {
      ...
      console.log(
        `Complex Query - Oracle: ${result.oracleTime}ms, PostgreSQL baseline: ${result.postgresqlTime}ms, Ratio: ${result.ratio}x`
      );
  ...
  describe('資料操作效能比較', () => {
    it('插入操作效能應符合150%限制', async () => {
      ...
      console.log(
        `Insert Operation - Oracle: ${result.oracleTime}ms, PostgreSQL baseline: ${result.postgresqlTime}ms, Ratio: ${result.ratio}x`
      );
    ...
    it('更新操作效能應符合150%限制', async () => {
      ...
      console.log(
        `Update Operation - Oracle: ${result.oracleTime}ms, PostgreSQL baseline: ${result.postgresqlTime}ms, Ratio: ${result.ratio}x`
      );
    ...
    it('交易操作效能應符合150%限制', async () => {
      ...
      const result = await benchmarkOperation(
        ...
        async () => {
          ...
          const transactionResult = await oracle.executeTransaction(
            async connection => {
              // 在交易中執行多個操作
      ...
      console.log(
        `Transaction Operation - Oracle: ${result.oracleTime}ms, PostgreSQL baseline: ${result.postgresqlTime}ms, Ratio: ${result.ratio}x`
      );
  ...
  describe('JSON處理效能比較', () => {
    it('JSON查詢效能應符合150%限制', async () => {
      ...
      console.log(
        `JSON Query - Oracle: ${result.oracleTime}ms, PostgreSQL baseline: ${result.postgresqlTime}ms, Ratio: ${result.ratio}x`
      );
  ...
  describe('整體效能總結', () => {
    it('所有操作的平均效能比例應符合要求', () => {
      // 計算所有基準測試的平均比例
      const averageRatio =
        benchmarkResults.reduce((sum, result) => sum + result.ratio, 0) /
        benchmarkResults.length;

      console.log(`\n📈 Overall Performance Summary:`);
      console.log(`Average performance ratio: ${averageRatio.toFixed(2)}x`);
      console.log(
        `Performance limit: ${BENCHMARK_CONFIG.MAX_ACCEPTABLE_RATIO}x`
      );
      ...
      console.log(
        `Pass rate: ${passRate.toFixed(1)}% (${passedTests}/${benchmarkResults.length})`
      );

      // 整體平均比例應該符合要求
      expect(averageRatio).toBeLessThanOrEqual(
        BENCHMARK_CONFIG.MAX_ACCEPTABLE_RATIO
      );
