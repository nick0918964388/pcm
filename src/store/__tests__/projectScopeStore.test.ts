...
describe('ProjectScopeStore', () => {
  ...
  describe('專案選擇功能', () => {
    it('應該能夠選擇專案', () => {
      ...
      expect(result.current.currentProjectPermission).toBe(ProjectPermission.EDITOR)
    ...
    it('應該能夠清除當前專案', () => {
      ...
      expect(result.current.currentProjectPermission).toBe(ProjectPermission.VIEWER)
      expect(mockSessionStorage.removeItem).toHaveBeenCalledWith('selectedProject')
    })
  })
  ...
  describe('最近存取專案管理', () => {
    ...
    it('應該限制最近專案記錄數量', () => {
      ...
      for (let i = 0; i < 25; i++) {
        const project = { ...mockProject1, id: `proj${i.toString().padStart(3, '0')}` }
        act(() => {
  ...
  describe('權限管理', () => {
    it('應該能夠檢查權限', () => {
      ...
      expect(result.current.hasPermission(ProjectPermission.SUPER_ADMIN)).toBe(false)
    })
    ...
    it('應該能夠更新當前專案權限', () => {
      ...
      expect(result.current.currentProjectPermission).toBe(ProjectPermission.ADMIN)
    })
  })
  ...
  describe('Utility Hooks', () => {
    ...
    it('useRecentProjects 應該返回限制數量的最近專案', () => {
      ...
      act(() => {
        ...
        storeResult.current.recordProjectAccess({ ...mockProject1, id: 'proj003' })
      })
    ...
    it('useFavoriteProjects 應該返回收藏相關功能', () => {
      ...
      expect(hookResult.current.favoriteProjectIds).not.toContain(mockProject1.id)
    })
