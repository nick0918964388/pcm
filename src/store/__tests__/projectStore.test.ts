...
describe('ProjectStore Search and Filter (US2)', () => {
  ...
  it('should search projects by query (AC2.1)', async () => {
    ...
    expect(
      result.current.filteredProjects.some(p => p.name.includes('台北'))
    ).toBe(true);
  });
  ...
  it('should filter projects by status (AC2.2)', async () => {
    ...
    expect(
      result.current.filteredProjects.every(
  ...
  it('should filter projects by progress range', async () => {
    ...
    expect(
      filteredProjects.every(p => p.progress >= 60 && p.progress <= 90)
    ).toBe(true);
  });
    ...
    expect(
      filteredProjects.every(
        p =>
          ...
          p.type === ProjectType.INFRASTRUCTURE
      )
    ).toBe(true);
  });
  ...
  it('should reset all filters', async () => {
    ...
    expect(result.current.filteredProjects.length).toBeGreaterThan(
...
describe('ProjectStore Sorting', () => {
  ...
  it('should sort projects by name ascending', () => {
    ...
    for (let i = 1; i < sortedNames.length; i++) {
      expect(
        sortedNames[i].localeCompare(sortedNames[i - 1], 'zh-TW')
  ...
  it('should sort projects by start date', () => {
    ...
    const sortedDates = result.current.filteredProjects.map(p =>
...
describe('ProjectStore Real-time Updates (US6 AC6.1)', () => {
  ...
  it('should update filtered projects after real-time update', async () => {
    ...
    expect(result.current.filteredProjects.length).toBeGreaterThanOrEqual(
      initialCount
    );
  });
...
describe('ProjectStore Selectors', () => {
  ...
  it('should get project statistics', () => {
    ...
    const { result } = renderHook(() => {
      ...
      return {
        total,
        activeCount: store.projects.filter(
          p => p.status === ProjectStatus.IN_PROGRESS
        ).length,
      };
    });
