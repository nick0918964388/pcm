...
describe('ProjectScopeStore WBS 管理擴展', () => {
  ...
  describe('WBS 樹狀結構管理', () => {
    ...
    it('應該能展開/摺疊節點', () => {
      ...
      expect(result.current.wbsManagement.expandedNodes.has(nodeId)).toBe(
        false
      );
    });
    ...
    it('應該支援多選節點', () => {
      ...
      expect(result.current.wbsManagement.selectedNodes.has(nodeId1)).toBe(
        true
      );
      expect(result.current.wbsManagement.selectedNodes.has(nodeId2)).toBe(
        true
      );
    });
  });
  ...
  describe('WBS 拖拽功能', () => {
    it('應該能開始拖拽', () => {
      ...
      expect(result.current.wbsManagement.dragState.draggedNode).toBe(
        draggedNode
      );
    });
    ...
    it('應該能設定放置目標', () => {
      ...
      expect(result.current.wbsManagement.dragState.dropTarget).toBe(
        dropTarget
      );
    });
