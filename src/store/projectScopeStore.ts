...
export interface ProjectScopeActions {
  ...
  /** 更新人員分頁 */
  updateMemberPagination: (pagination: {
    page?: number;
    pageSize?: number;
  }) => void;
  /** 檢查人員快取是否有效 */
 ...
 * 專案範疇管理 Store
 */
export const useProjectScopeStore = create<
  ProjectScopeState & ProjectScopeActions
>()(
  devtools(
    persist(
      (set, get) => ({
        ...
        switchToProject: async (projectId: string) => {
          const state = get();
          const projectRecord = state.recentProjects.find(
            ...
            state => {
              ...
              if (existingIndex >= 0) {
                ...
                const updatedRecord = {
                  ...
                  accessCount:
                    state.recentProjects[existingIndex].accessCount + 1,
                };
        ...
        updateUserPreferences: (
          preferences: Partial<UserProjectPreferences>
        ) => {
          set(
            state => ({
              ...state,
        ...
        hasPermission: (permission: ProjectPermission) => {
          ...
          return (
            permissionLevels[currentPermission] >= permissionLevels[permission]
          );
        },
          ...
          set(
            state => {
              const newExpandedNodes = new Set(
                state.wbsManagement.expandedNodes
              );
            ...
            state => ({
              ...
              wbsManagement: {
                ...
                viewState: {
                  ...state.wbsManagement.viewState,
                  filterPanelOpen:
                    !state.wbsManagement.viewState.filterPanelOpen,
                },
              },
            }),
            false,
...
export const useFavoriteProjects = () => {
  const favoriteProjectIds = useProjectScopeStore(
    state => state.favoriteProjects
  );
  const toggleFavorite = useProjectScopeStore(
    state => state.toggleProjectFavorite
  );
...
export const useUserPreferences = () => {
  ...
  const updatePreferences = useProjectScopeStore(
    state => state.updateUserPreferences
  );
  const resetPreferences = useProjectScopeStore(
    state => state.resetUserPreferences
  );
...
export const useProjectMembers = () => {
  ...
  return {
    ...
    pagination: state.projectMembers?.pagination || {
      page: 1,
      pageSize: 10,
      total: 0,
      totalPages: 0,
...
export const useWBSTree = () => {
  ...
  return {
    ...
    dragState: state.wbsManagement?.dragState || {
      isDragging: false,
      draggedNode: null,
      dropTarget: null,
    },
    viewState: state.wbsManagement?.viewState || {
      showCompleted: true,
      viewMode: 'tree',
      filterPanelOpen: false,
    },
