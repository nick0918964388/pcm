...
async function createModule(title: string, level: string, duration: string) {
  try {
    ...
    logError(`建立模組失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}

// 更新培訓進度命令
async function updateProgress(userId: string, moduleId: string, percentage: string) {
  try {
    ...
    logError(`更新進度失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
  ...
  try {
    ...
    if (report.userProgress.length > 0) {
      ...
      report.userProgress.forEach(user => {
        log(`  ${user.userId}: 完成${user.completedModules}個模組 (${user.totalProgress}%)`)
      })
    }
    ...
    if (report.moduleStatistics.length > 0) {
      ...
      report.moduleStatistics.forEach(module => {
        log(`  ${module.moduleId}: ${module.enrollments}人註冊, ${module.completions}人完成, 評分${module.averageRating}`)
      })
    }
...
async function recommendPath(userId: string, role: string, experience: string) {
  try {
    ...
    logInfo('分析用戶需求...')
    const recommendations = await trainingManager.recommendTrainingPath(userProfile)
    ...
    logError(`路徑推薦失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function generateAPIDocs() {
  try {
    ...
    logError(`API文件生成失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function createTroubleshootingGuide() {
  try {
    ...
    logError(`故障排除指南建立失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function generateRunbook() {
  try {
    ...
    logInfo('生成執行手冊...')
    const result = await documentationEngine.generateMigrationRunbook(migrationSteps)
    ...
    logError(`執行手冊生成失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
try {
  ...
  console.error('回滾失敗:', error)
    ...
    logError(`程式碼範例建立失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function searchDocs(query: string) {
  try {
    ...
    log(`找到 ${result.totalResults} 個結果 (搜尋時間: ${result.searchTime}ms)`)
    ...
    logError(`搜尋失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function storeArticle(title: string, category: string, content: string) {
  try {
    ...
    logError(`儲存文章失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function searchKnowledge(terms: string[]) {
  try {
    ...
    log(`找到 ${result.results.length} 個結果 (搜尋時間: ${result.searchTime}ms)`)
    ...
    logError(`知識搜尋失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function generateKnowledgeReport() {
  try {
    ...
    if (report.statistics.topCategories.length > 0) {
      log(`  熱門類別: ${report.statistics.topCategories.slice(0, 3).join(', ')}`)
    }

    if (report.topArticles.length > 0) {
      log('\n🔥 熱門文章:', 'yellow')
      report.topArticles.slice(0, 5).forEach((article, index) => {
        log(`  ${index + 1}. ${article.title} (${article.views} 次瀏覽, ${article.rating.toFixed(1)} 分)`)
      })
    }
    ...
    logError(`知識報告生成失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function createGuide(title: string, description: string) {
  try {
    ...
    logError(`建立指南失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}

// 建立團隊工作空間命令
async function createWorkspace(name: string, description: string, members: string[]) {
  try {
    ...
    logError(`建立工作空間失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function trackTeamProgress(teamId: string) {
  try {
    ...
    logError(`團隊進度追蹤失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function generateTeamReport(teamId: string, reportType: string) {
  try {
    ...
    logInfo('生成報告...')
    const report = await teamCollaboration.generateTeamReport(teamId, reportType)
    ...
    logError(`團隊報告生成失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
trainingCommand
  ...
  .description('建立培訓模組 (level: beginner|intermediate|advanced, duration: 分鐘)')
  .action(createModule)
