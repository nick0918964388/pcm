...
async function checkConsistency(tableName?: string) {
  try {
    ...
    if (tableName) {
      ...
      if (report.tableResults.length > 0) {
        ...
        report.tableResults.forEach(table => {
          ...
          log(
            `  ${status} ${table.tableName}: PG=${table.postgresqlCount}, Oracle=${table.oracleCount}`
          );
        });
      }
    }
    ...
    logError(
      `一致性檢查失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function compareSchemas(tableName: string) {
  try {
    ...
    if (result.differences.length > 0) {
      log('\n📊 結構差異:', 'yellow');
      log(
        `${'欄位'.padEnd(20)} ${'PostgreSQL'.padEnd(15)} ${'Oracle'.padEnd(15)} ${'相容性'.padEnd(8)}`
      );
      log('-'.repeat(70));
    ...
    logError(
      `結構比較失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function scheduleSync(
  ...
  try {
    logStep(
      `設定同步排程: ${tables.join(', ')} (間隔: ${interval}, 方向: ${direction})`
    );
    ...
    logError(
      `排程設定失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function executeSchedule(scheduleId: string) {
  try {
      ...
      if (result.syncResults.length > 0) {
        ...
        result.syncResults.forEach((syncResult, index) => {
          if (syncResult.success) {
            log(
              `  ✅ 同步 ${index + 1}: ${syncResult.recordsSynchronized} 筆記錄`
            );
          } else {
    ...
    logError(
      `排程執行失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function incrementalSync(tableName: string, hours: number = 24) {
  try {
    ...
    logError(
      `增量同步失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
program
  ...
  .description(
    '同步表格資料 (方向: postgresql-to-oracle|oracle-to-postgresql|bidirectional)'
  )
  .action(syncTable);
