...
async function runMigrations() {
  try {
    ...
    logError(
      `遷移執行錯誤：${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
async function showVersion() {
  try {
    ...
    logError(
      `無法獲取版本：${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function showHistory() {
  try {
    ...
    logError(
      `無法獲取遷移歷史：${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function loadTestData() {
  try {
    ...
    logError(
      `測試資料載入錯誤：${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
async function cleanTestData() {
  try {
    ...
    logError(
      `測試資料清理錯誤：${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
async function validateTestData() {
  try {
    ...
    logError(
      `測試資料驗證錯誤：${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
async function analyzeStats() {
  try {
    ...
    log('\n表格統計資訊：', 'cyan');
    log(
      `${'表格名稱'.padEnd(25)} ${'行數'.padEnd(10)} ${'大小(MB)'.padEnd(10)} 最後分析時間`
    );
    log('-'.repeat(70));

    stats.forEach(stat => {
      const lastAnalyzed = stat.lastAnalyzed
        ? stat.lastAnalyzed.toLocaleDateString()
        : '從未';
    ...
    logError(
      `統計資訊分析錯誤：${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function updateStats() {
  try {
    ...
    logError(
      `統計資訊更新錯誤：${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
async function rebuildIndexes() {
  try {
    ...
    logError(
      `索引重建錯誤：${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
migrationCmd.command('run').description('執行資料庫遷移').action(runMigrations);
...
migrationCmd.command('history').description('顯示遷移歷史').action(showHistory);

// 測試資料相關命令
const testDataCmd = program.command('test-data').description('測試資料管理');

testDataCmd.command('load').description('載入測試資料').action(loadTestData);

testDataCmd.command('clean').description('清理測試資料').action(cleanTestData);
