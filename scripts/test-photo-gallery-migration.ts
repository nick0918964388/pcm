  ...
  try {
    ...
    for (let i = 0; i < createStatements.length; i++) {
      try {
        ...
        console.warn(`  ⚠️ 語句 ${i + 1} 執行失敗: ${(error as Error).message}`);
      }
    }

    console.log(`✅ 資料庫架構建立完成 (${successCount}/${createStatements.length} 成功)\n`);
...
async function testPhotoAlbumsTable(pool: Pool): Promise<TestResult> {
  try {
    ...
    const expectedColumns = ['id', 'project_id', 'name', 'description', 'cover_photo_id', 'photo_count', 'created_at', 'updated_at', 'deleted_at', 'created_by'];

    const missingColumns = expectedColumns.filter(col => !columns.includes(col));

    if (missingColumns.length === 0) {
      return { success: true, message: `相簿表結構正確 (${columns.length} 欄位)` };
    } else {
      return { success: false, message: `相簿表缺少欄位: ${missingColumns.join(', ')}` };
    }
  } catch (error) {
    return { success: false, message: `相簿表檢查失敗: ${(error as Error).message}` };
  }
}
...
async function testPhotosTable(pool: Pool): Promise<TestResult> {
  try {
    ...
    const columns = result.rows.map(r => r.column_name);
    const expectedColumns = ['id', 'album_id', 'file_name', 'file_path', 'file_size', 'mime_type', 'width', 'height', 'thumbnail_path', 'uploaded_by', 'uploaded_at', 'metadata', 'deleted_at'];
    ...
    if (missingColumns.length === 0 && isJsonb) {
      return { success: true, message: `照片表結構正確 (${columns.length} 欄位, metadata為JSONB)` };
    } else {
      const issues = [];
      if (missingColumns.length > 0) issues.push(`缺少欄位: ${missingColumns.join(', ')}`);
      if (!isJsonb) issues.push('metadata欄位類型不是JSONB');
      return { success: false, message: `照片表問題: ${issues.join('; ')}` };
    }
  } catch (error) {
    return { success: false, message: `照片表檢查失敗: ${(error as Error).message}` };
  }
}
...
async function testPhotoVersionsTable(pool: Pool): Promise<TestResult> {
  try {
    ...
    const columns = result.rows.map(r => r.column_name);
    const expectedColumns = ['id', 'photo_id', 'version_type', 'file_path', 'width', 'height', 'file_size'];

    const missingColumns = expectedColumns.filter(col => !columns.includes(col));

    if (missingColumns.length === 0) {
      return { success: true, message: `照片版本表結構正確 (${columns.length} 欄位)` };
    } else {
      return { success: false, message: `照片版本表缺少欄位: ${missingColumns.join(', ')}` };
    }
  } catch (error) {
    return { success: false, message: `照片版本表檢查失敗: ${(error as Error).message}` };
  }
}
...
async function testIndexes(pool: Pool): Promise<TestResult> {
  try {
    ...
    if (indexCount >= 5) { // 至少應該有主鍵 + 一些其他索引
      return { success: true, message: `索引建立正確 (${indexCount} 個索引)` };
    } else {
      return { success: false, message: `索引數量不足: ${indexCount} 個 (預期至少5個)` };
    }
  } catch (error) {
    return { success: false, message: `索引檢查失敗: ${(error as Error).message}` };
  }
}
...
async function testSoftDelete(pool: Pool): Promise<TestResult> {
  try {
    ...
    if (result.rows.length === 2) {
      return { success: true, message: '軟刪除欄位 (deleted_at) 存在於所有必要表格' };
    } else {
      return { success: false, message: `軟刪除欄位缺失: 找到 ${result.rows.length} 個，預期 2 個` };
    }
  } catch (error) {
    return { success: false, message: `軟刪除檢查失敗: ${(error as Error).message}` };
  }
}
