...
async function showStatus() {
  try {
    ...
    log(
      `整體狀態: ${status.overallStatus}`,
      statusColors[status.overallStatus]
    );
    ...
    log('\n📈 進度資訊', 'bright');
    log(
      `完成階段: ${progress.completedPhases}/${progress.totalPhases} (${progress.progressPercentage}%)`
    );
    ...
    logError(
      `獲取狀態失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function listPhases() {
  try {
    ...
    log('='.repeat(100));
    log(
      `${'ID'.padEnd(20)} ${'名稱'.padEnd(20)} ${'狀態'.padEnd(12)} ${'預估時間'.padEnd(8)} ${'依賴項'.padEnd(20)}`
    );
    log('-'.repeat(100));
    ...
    phases.forEach(phase => {
      ...
      const dependencies =
        phase.dependencies.length > 0 ? phase.dependencies.join(', ') : '無';
    ...
    logError(
      `列出階段失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function executePhase(phaseId: string) {
  try {
    ...
    logError(
      `執行階段失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function executeParallel(phaseIds: string[]) {
  try {
    ...
    // 檢查是否可以並行執行
    const canExecuteInParallel =
      await parallelManager.canExecuteInParallel(phaseIds);
    ...
    results.forEach(result => {
      if (result.success) {
        logSuccess(
          `${result.phaseId}: 成功 (${Math.round(result.duration / 1000)}秒)`
        );
      } else {
    ...
    logError(
      `並行執行失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function switchEnvironment(targetEnv: 'postgresql' | 'oracle') {
  try {
    ...
    logError(
      `環境切換失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
  ...
  try {
    ...
    await statusTracker.recordEvent({
      ...
      metadata: {
        ...
        testCount: result.validationResults.length,
      },
    });
  } catch (error) {
    logError(
      `驗證檢查點失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function showTimeline() {
  try {
    ...
    logError(
      `獲取時間軸失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function resetMigration() {
  try {
    ...
    logError(
      `重置失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
// 狀態和資訊命令
program.command('status').description('顯示遷移狀態').action(showStatus);
...
// 管理命令
program.command('reset').description('重置遷移狀態').action(resetMigration);
