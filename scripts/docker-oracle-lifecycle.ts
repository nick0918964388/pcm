...
async function runCommand(command: string, description: string) {
  try {
    ...
    logError(`${description}失敗: ${error instanceof Error ? error.message : String(error)}`)
...
async function getContainerStatus() {
  try {
    const { stdout } = await execAsync(`docker ps -a --format "{{json .}}" --filter "name=${CONTAINER_NAME}"`)
    if (!stdout.trim()) {
    ...
    logError(`檢查容器狀態失敗: ${error instanceof Error ? error.message : String(error)}`)
...
async function waitForOracleReady(timeoutMinutes = 5): Promise<boolean> {
  ...
  while (Date.now() - startTime < timeoutMs) {
    try {
      // 檢查容器健康狀態
      const { stdout } = await execAsync(`docker inspect ${CONTAINER_NAME} --format='{{.State.Health.Status}}'`)
      ...
      // 嘗試連線測試
      try {
        await execAsync(`docker exec ${CONTAINER_NAME} sqlplus -L system/Oracle123@//localhost:1521/XE @/dev/null <<< "SELECT 1 FROM dual; EXIT;"`)
...
async function statusOracle() {
  ...
  if (status.state === 'running') {
    try {
      ...
      // 測試連線
      try {
        await execAsync(`docker exec ${CONTAINER_NAME} sqlplus -L system/Oracle123@//localhost:1521/XE @/dev/null <<< "SELECT 1 FROM dual; EXIT;" 2>/dev/null`)
        log('   資料庫連線: 成功', 'green')
      } catch {
        log('   資料庫連線: 失敗', 'red')
      }
...
program
  .command('start')
  .description('啟動Oracle開發環境')
  .action(startOracle)

program
  .command('stop')
  .description('停止Oracle開發環境')
  .action(stopOracle)
