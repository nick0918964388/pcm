...
async function executeRollback(snapshotId: string, targetDatabase: string) {
  try {
    ...
    logError(
      `回滾失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function estimateRollback(tables: string[]) {
  try {
    ...
    logError(
      `估算失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function diagnoseIssues() {
  try {
    ...
    logError(
      `診斷失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function checkConnectivity() {
  try {
    ...
    logError(
      `連線檢查失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function generateReport() {
  try {
    ...
    log(
      `\n整體健康狀態: ${result.summary.overallHealth}`,
      healthColors[result.summary.overallHealth]
    );
    ...
    log(
      `  PostgreSQL: ${conn.postgresql.connected ? '✅' : '❌'} (${conn.postgresql.responseTime}ms)`
    );
    log(
      `  Oracle: ${conn.oracle.connected ? '✅' : '❌'} (${conn.oracle.responseTime}ms)`
    );
...
async function emergencyRecover() {
  try {
    ...
    logError(
      `緊急恢復失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function restoreMinimal() {
  try {
    ...
    logError(
      `最小功能恢復失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function detectAutoRecovery() {
  try {
    ...
    if (result.opportunities.length === 0) {
      ...
      result.opportunities.forEach((opportunity, index) => {
        ...
        if (opportunity.requiredActions.length > 0) {
          log(`   所需行動:`);
          opportunity.requiredActions.forEach(action =>
            log(`     - ${action}`)
          );
        }
      });
    ...
    logError(
      `自動恢復檢測失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function executeAutoRecovery(recoveryType: string) {
  try {
    ...
    logError(
      `自動恢復失敗: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
// 回滾命令群組
const rollbackCommand = program.command('rollback').description('回滾管理命令');
...
// 自動恢復命令群組
const autoCommand = program.command('auto').description('自動恢復命令');
...
autoCommand
  ...
  .description(
    '執行自動恢復 (類型: connection_retry|data_sync_retry|partial_rollback|emergency_isolation)'
  )
