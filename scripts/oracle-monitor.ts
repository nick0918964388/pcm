...
async function healthCheck() {
  try {
    ...
    logError(`健康檢查失敗: ${error instanceof Error ? error.message : String(error)}`)
    process.exit(1)
  }
}
...
async function performanceMetrics() {
  try {
    ...
    logError(`獲取效能指標失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function slowQueries(minTime?: string) {
  try {
    ...
    logError(`獲取慢查詢失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function sessions() {
  try {
    ...
    log('\n👥 當前會話', 'bright')
    log('=' .repeat(100))
    log(`${'會話ID'.padEnd(8)} ${'用戶'.padEnd(15)} ${'狀態'.padEnd(10)} ${'登入時間'.padEnd(20)} ${'最後活動'.padEnd(20)}`)
    log('-' .repeat(100))
    ...
    logError(`獲取會話資訊失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function tablespaces() {
  try {
    ...
    log('\n💾 表空間使用率', 'bright')
    log('=' .repeat(80))
    log(`${'名稱'.padEnd(20)} ${'總大小(MB)'.padEnd(12)} ${'已使用(MB)'.padEnd(12)} ${'使用率'.padEnd(8)} ${'狀態'.padEnd(10)}`)
    log('-' .repeat(80))

    tablespaces.forEach(ts => {
      const usageColor = ts.usagePercentage > 90 ? 'red' :
                        ts.usagePercentage > 80 ? 'yellow' : 'green'
    ...
    logError(`獲取表空間資訊失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function locks() {
  try {
    ...
    log('\n🔒 鎖等待情況', 'bright')
    log('=' .repeat(80))
    log(`${'會話ID'.padEnd(8)} ${'等待事件'.padEnd(30)} ${'等待時間(s)'.padEnd(12)} ${'對象'.padEnd(15)} ${'鎖模式'.padEnd(10)}`)
    log('-' .repeat(80))
    ...
    logError(`檢查鎖等待失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function blocking() {
  try {
    ...
    log('\n🚫 阻塞會話', 'bright')
    log('=' .repeat(80))
    log(`${'阻塞者'.padEnd(8)} ${'被阻塞'.padEnd(8)} ${'鎖類型'.padEnd(10)} ${'對象'.padEnd(20)} ${'等待時間(s)'.padEnd(12)}`)
    log('-' .repeat(80))
    ...
    logError(`檢查阻塞會話失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function performanceReport(hours?: string) {
  try {
    ...
    log(`統計期間: ${report.timeRange.start.toLocaleString()} ~ ${report.timeRange.end.toLocaleString()}`)

    if (report.statistics.length > 0) {
      log('\n📊 系統統計', 'cyan')
      report.statistics.forEach(stat => {
        log(`  ${stat.metric}: ${stat.value.toLocaleString()} ${stat.unit || ''}`)
      })
    }
    ...
    if (report.topSQLStatements.length > 0) {
      ...
      report.topSQLStatements.slice(0, 5).forEach((sql, index) => {
        log(`  ${index + 1}. 執行次數: ${sql.executions}, 總時間: ${sql.totalTime.toFixed(2)}s`)
        log(`     SQL: ${sql.sqlText}`, 'magenta')
      })
    }

  } catch (error) {
    logError(`生成效能報告失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function explainPlan(sqlText: string) {
  try {
    ...
    logError(`分析執行計畫失敗: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
async function monitor(interval?: string) {
  ...
  const runMonitoring = async () => {
    ...
    try {
      ...
      logError(`監控錯誤: ${error instanceof Error ? error.message : String(error)}`)
    }
