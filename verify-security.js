...
testFilenames.forEach(filename => {
  ...
  const hasDangerousExt = ['.exe', '.bat', '.js', '.php'].some(ext => filename.endsWith(ext))

  const isValid = !hasUnsafeChars && !hasPathTraversal && !hasDangerousExt

  console.log(`  ${isValid ? '✅' : '❌'} ${filename} ${isValid ? '(安全)' : '(危險)'}`)
})
...
testTypes.forEach(type => {
  ...
  console.log(`  ${isAllowed ? '✅' : '❌'} ${type} ${isAllowed ? '(允許)' : '(拒絕)'}`)
})
...
testSizes.forEach(({ size, name }) => {
  ...
  console.log(`  ${isValid ? '✅' : '❌'} ${name} ${isValid ? '(允許)' : '(拒絕)'}`)
})
...
function checkRateLimit(userId, maxRequests = 5, windowMs = 60000) {
  const now = Date.now()
  const record = rateLimitStore.get(userId) || { count: 0, resetTime: now + windowMs }
...
for (let i = 1; i <= 7; i++) {
  ...
  console.log(`  請求 ${i}: ${result.allowed ? '✅ 允許' : '❌ 拒絕'} (剩餘: ${result.remaining})`)
}
...
function generateSecurePath(projectId, userId, filename) {
  ...
  // 雜湊項目和用戶ID
  const hashedProject = crypto.createHash('sha256').update(projectId).digest('hex').substring(0, 8)
  const hashedUser = crypto.createHash('sha256').update(userId).digest('hex').substring(0, 8)
...
function createBasicSignedUrl(url, expiresIn) {
  ...
  const signature = crypto.createHmac('sha256', 'test-secret').update(payloadStr).digest('hex')
